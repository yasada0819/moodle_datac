...
use mod_datac\external\database_summary_exporter;
use mod_datac\external\record_exporter;
use mod_datac\external\content_exporter;
use mod_datac\external\field_exporter;
 ...
 * @since      Moodle 2.9
 */
class mod_datac_external extends external_api {
    ...
    public static function get_databases_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            $databases = get_all_instances_in_courses("datac", $dbcourses);
    ...
    protected static function validate_database($databaseid) {
        ...
        $database = $DB->get_record('datac', array('id' => $databaseid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($database, 'datac');
    ...
    public static function view_database($databaseid) {
        ...
        datac_view($database, $course, $cm, $context);
    ...
    public static function get_data_access_information($databaseid, $groupid = 0) {
        ...
        $result['canaddentry'] = datac_user_can_add_entry($database, $groupid, $groupmode, $context);

        // Now capabilities.
        $result['canmanageentries'] = has_capability('mod/datac:manageentries', $context);
        $result['canapprove'] = has_capability('mod/datac:approve', $context);

        // Now time access restrictions.
        list($result['timeavailable'], $warnings) = datac_get_time_availability_status($database, $result['canmanageentries']);

        // Other information.
        $result['numentries'] = datac_numentries($database);
        $result['entrieslefttoadd'] = datac_get_entries_left_to_add($database, $result['numentries'], $result['canmanageentries']);
        $result['entrieslefttoview'] = datac_get_entries_left_to_view($database, $result['numentries'], $result['canmanageentries']);
        $result['inreadonlyperiod'] = datac_in_readonly_period($database);
    ...
    public static function get_entries($databaseid, $groupid = 0, $returncontents = false, $sort = null, $order = null,
        ...
        datac_require_time_available($database, null, $context);
        ...
        if (!empty($params['groupid'])) {
            ...
            if ($groupmode = groups_get_activity_groupmode($cm)) {
                // We don't need to validate a possible groupid = 0 since it would be handled by datac_search_entries.
                $groupid = groups_get_activity_group($cm);
            } else {
                $groupid = 0;
            }
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datac_search_entries($database, $cm, $context, 'list', $groupid, '', $params['sort'], $params['order'],
        ...
        foreach ($records as $record) {
            ...
            $contents = $DB->get_records('datac_content', array('recordid' => $record->id));
        ...
        $totalfilesize = 0;
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'mod_datac', 'content');
        ...
        if ($params['returncontents']) {
            ...
            datac_print_template('listtemplate', $records, $database, '', $page, false);
    ...
    public static function get_entry($entryid, $returncontents = false) {
        ...
        $record = $DB->get_record('datac_records', array('id' => $params['entryid']), '*', MUST_EXIST);
        ...
        datac_require_time_available($database, $canmanageentries);
        ...
        if (!datac_can_view_record($database, $record, $record->groupid, $canmanageentries)) {
            throw new moodle_exception('notapproved', 'datac');
        }

        $related = array('context' => $context, 'database' => $database, 'user' => null);
        if ($params['returncontents']) {
            $related['contents'] = $DB->get_records('datac_content', array('recordid' => $record->id));
        } else {
            $related['contents'] = null;
        }
        $exporter = new record_exporter($record, $related);
        ...
        $result = array(
            'entry' => $entry,
            'ratinginfo' => \core_rating\external\util::get_rating_info($database, $context, 'mod_datac', 'entry', array($record)),
            'warnings' => $warnings
        );
        // Check if we should return the entry rendered.
        if ($params['returncontents']) {
            $records = [$record];
            $result['entryviewcontents'] = datac_print_template('singletemplate', $records, $database, '', 0, true);
        }
    ...
    public static function get_fields($databaseid) {
        ...
        $fieldinstances = datac_get_field_instances($database);
    ...
    public static function search_entries($databaseid, $groupid = 0, $returncontents = false, $search = '', $advsearch = [],
        ...
        if (!empty($params['advsearch'])) {
            ...
            list($searcharray, $params['search']) = datac_build_search_array($database, false, [], $defaults, $fn, $ln);
        } else {
            $advanced = null;
            $searcharray = null;
        }
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datac_search_entries($database, $cm, $context, 'list', $groupid, $params['search'], $params['sort'], $params['order'],
    ...
    public static function approve_entry($entryid, $approve = true) {
        ...
        require_capability('mod/datac:approve', $context);

        datac_approve_entry($record->id, $params['approve']);
    ...
    public static function delete_entry($entryid) {
        ...
        if (datac_user_can_manage_entry($record, $database, $context)) {
            datac_delete_record($record->id, $database, $course->id, $cm->id);
        } else {
            throw new moodle_exception('noaccess', 'datac');
        }
    ...
    public static function add_entry_parameters() {
        return new external_function_parameters(
            array(
                ...
                'groupid' => new external_value(PARAM_INT, 'Group id, 0 means that the function will determine the user group',
                                                   VALUE_DEFAULT, 0),
                'datac' => new external_multiple_structure(
                    new external_single_structure(
                        array(
    ...
    public static function add_entry($databaseid, $groupid, $data) {
        ...
        $params = array('databaseid' => $databaseid, 'groupid' => $groupid, 'datac' => $data);
        ...
        if (!datac_user_can_add_entry($database, $groupid, $groupmode, $context)) {
            throw new moodle_exception('noaccess', 'datac');
        }

        // Prepare the data as is expected by the API.
        $datarecord = new stdClass;
        foreach ($params['datac'] as $data) {
        ...
        $fields = $DB->get_records('datac_fields', array('dataid' => $database->id));
        $processeddata = datac_process_submission($database, $fields, $datarecord);
        ...
        $newentryid = 0;
        if ($processeddata->validated && $recordid = datac_add_record($database, $groupid)) {
            $newentryid = $recordid;
            // Now populate the fields contents of the new record.
            datac_add_fields_contents_to_new_record($database, $context, $recordid, $fields, $datarecord, $processeddata);
        }
    ...
    public static function update_entry($entryid, $data) {
        ...
        $params = array('entryid' => $entryid, 'datac' => $data);
        ...
        if ($processeddata->validated) {
            ...
            datac_update_record_fields_contents($database, $record, $context, $datarecord, $processeddata);
            $updated = true;
        }
