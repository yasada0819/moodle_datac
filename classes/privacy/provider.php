...
/**
 * Privacy Subsystem implementation for mod_datac.
 *
 * @package    mod_datac
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        $collection->add_database_table(
            'datac_records',
            [
                'userid' => 'privacy:metadata:datac_records:userid',
                'groupid' => 'privacy:metadata:datac_records:groupid',
                'timecreated' => 'privacy:metadata:datac_records:timecreated',
                'timemodified' => 'privacy:metadata:datac_records:timemodified',
                'approved' => 'privacy:metadata:datac_records:approved',
            ],
            'privacy:metadata:datac_records'
        );
        $collection->add_database_table(
            'datac_content',
            [
                'fieldid' => 'privacy:metadata:datac_content:fieldid',
                'content' => 'privacy:metadata:datac_content:content',
                'content1' => 'privacy:metadata:datac_content:content1',
                'content2' => 'privacy:metadata:datac_content:content2',
                'content3' => 'privacy:metadata:datac_content:content3',
                'content4' => 'privacy:metadata:datac_content:content4',
            ],
            'privacy:metadata:datac_content'
        );

        // Link to subplugins.
        $collection->add_plugintype_link('datacfield', [], 'privacy:metadata:datacfieldnpluginsummary');
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $sql = "SELECT c.id
            ...
            INNER JOIN {datac} d ON d.id = cm.instance
            INNER JOIN {datac_records} dr ON dr.dataid = d.id
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $sql = "SELECT dr.userid
                  ...
                  JOIN {datac} d ON d.id = cm.instance
                  JOIN {datac_records} dr ON dr.dataid = d.id
        ...
        \core_comment\privacy\provider::get_users_in_context_from_sql($userlist, 'com', 'mod_datac', 'database_entry', $context->id);
        ...
        \core_rating\privacy\provider::get_users_in_context_from_sql($userlist, 'rat', 'mod_datac', 'entry', $sql, $params);
    }
     ...
     */
    protected static function export_datac_content($context, $recordobj, $fieldobj, $contentobj) {
        $value = (object)[
            'field' => [
                // Name and description are displayed in mod_datac without applying format_string().
                'name' => $fieldobj->name,
        ...
        $classname = manager::get_provider_classname_for_component('datacfield_' . $fieldobj->type);
        if (class_exists($classname) && is_subclass_of($classname, datacfield_provider::class)) {
            component_class_callback($classname, 'export_datac_content',
                [$context, $recordobj, $fieldobj, $contentobj, $value]);
        } else {
            // Data field plugin does not implement datacfield_provider, just export default value.
            writer::with_context($context)->export_data([$recordobj->id, $contentobj->id], $value);
        }
        writer::with_context($context)->export_area_files([$recordobj->id, $contentobj->id], 'mod_datac',
            'content', $contentobj->id);
    }

    /**
     * SQL query that returns all fields from {datac_content}, {datac_fields} and {datac_records} tables
     *
     * @return string
     */
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        $sql = "SELECT cm.id AS cmid, d.name AS dataname, cm.course AS courseid, " . self::sql_fields() . "
                ...
                JOIN {datac} d ON d.id = cm.instance
                JOIN {datac_records} dr ON dr.dataid = d.id
                JOIN {datac_content} dc ON dc.recordid = dr.id
                JOIN {datac_fields} df ON df.id = dc.fieldid
        ...
        $rs = $DB->get_recordset_sql($sql, $contextparams + ['contextlevel' => CONTEXT_MODULE,
                'modname' => 'datac', 'userid' => $user->id, 'userid1' => $user->id, 'commentarea' => 'database_entry',
                'userid2' => $user->id, 'ratingarea' => 'entry', 'moddata' => 'mod_datac']);
        ...
        foreach ($rs as $row) {
            ...
            if (!$recordobj || $row->recordid != $recordobj->id) {
                ...
                self::export_datac_record($context, $user, $recordobj);
            ...
            self::export_datac_content($context, $recordobj, $fieldobj, $contentobj);
        }
        $rs->close();
        self::export_datac_record($context, $user, $recordobj);
        self::export_data($context, $user);
    ...
    /**
     * Export one entry in the database activity module (one record in {datac_records} table)
     *
     * @param \context $context
     * @param \stdClass $user
     * @param \stdClass $recordobj
     */
    protected static function export_datac_record($context, $user, $recordobj) {
        if (!$recordobj) {
            return;
        }
        $data = [
        ...
        \core_tag\privacy\provider::export_item_tags($user->id, $context, [$recordobj->id],
            'mod_datac', 'datac_records', $recordobj->id);
        ...
        \core_comment\privacy\provider::export_comments($context, 'mod_datac', 'database_entry', $recordobj->id,
        ...
        \core_rating\privacy\provider::export_area_ratings($user->id, $context, [$recordobj->id], 'mod_datac', 'entry',
            $recordobj->id, $recordobj->userid != $user->id);
    }
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $sql = "SELECT " . self::sql_fields() . "
                ...
                JOIN {datac} d ON d.id = cm.instance
                JOIN {datac_records} dr ON dr.dataid = d.id
                LEFT JOIN {datac_content} dc ON dc.recordid = dr.id
                LEFT JOIN {datac_fields} df ON df.id = dc.fieldid
                WHERE cm.id = :cmid
                ORDER BY dr.id";
        $rs = $DB->get_recordset_sql($sql, ['cmid' => $context->instanceid, 'modname' => 'datac']);
        foreach ($rs as $row) {
            self::mark_datac_content_for_deletion($context, $row);
            $recordstobedeleted[$row->recordid] = $row->recordid;
        }
        $rs->close();

        self::delete_datac_records($context, $recordstobedeleted);
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            $sql = "SELECT " . self::sql_fields() . "
                ...
                JOIN {modules} m ON m.id = cm.module AND m.name = :modname
                JOIN {datac} d ON d.id = cm.instance
                JOIN {datac_records} dr ON dr.dataid = d.id AND dr.userid = :userid
                LEFT JOIN {datac_content} dc ON dc.recordid = dr.id
                LEFT JOIN {datac_fields} df ON df.id = dc.fieldid
        ...
        \core_comment\privacy\provider::delete_comments_for_user($contextlist, 'mod_datac', 'database_entry');
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        $sql = "SELECT " . self::sql_fields() . "
                  ...
                  JOIN {modules} m ON m.id = cm.module AND m.name = :modname
                  JOIN {datac} d ON d.id = cm.instance
                  JOIN {datac_records} dr ON dr.dataid = d.id AND dr.userid {$userinsql}
             LEFT JOIN {datac_content} dc ON dc.recordid = dr.id
             LEFT JOIN {datac_fields} df ON df.id = dc.fieldid
                 WHERE ctx.id = :ctxid AND ctx.contextlevel = :contextlevel
              ORDER BY dr.id";
        ...
        \core_comment\privacy\provider::delete_comments_for_users($userlist, 'mod_datac', 'database_entry');

        // We do not delete ratings made by users on other records because it may change grades.
    }

    /**
     * Marks a datac_record/datac_content for deletion
     *
     * Also invokes callback from datacfield plugin in case it stores additional data that needs to be deleted
     *
     * @param \context $context
     * @param \stdClass $row result of SQL query - tables datac_content, datac_record, datac_fields join together
     */
    protected static function mark_datac_content_for_deletion($context, $row) {
        $recordobj = self::extract_object_from_record($row, 'record', ['dataid' => $row->dataid]);
        if ($row->contentid && $row->fieldid) {
            ...
            // Allow datacfield plugin to implement their own deletion.
            $classname = manager::get_provider_classname_for_component('datacfield_' . $fieldobj->type);
            if (class_exists($classname) && is_subclass_of($classname, datacfield_provider::class)) {
                component_class_callback($classname, 'delete_datac_content',
     ...
     *
     * Should be executed after all records were marked by {@link mark_datac_content_for_deletion()}
     *
     * Deletes records from datac_content and datac_records tables, associated files, tags, comments and ratings.
     *
     * @param \context $context
     * @param array $recordstobedeleted list of ids of the data records that need to be deleted
     */
    protected static function delete_datac_records($context, $recordstobedeleted) {
        global $DB;
        ...
        // Delete files.
        get_file_storage()->delete_area_files_select($context->id, 'mod_datac', 'datac_records',
            "IN (SELECT dc.id FROM {datac_content} dc WHERE dc.recordid $sql)", $params);
        // Delete from datac_content.
        $DB->delete_records_select('datac_content', 'recordid ' . $sql, $params);
        // Delete from datac_records.
        $DB->delete_records_select('datac_records', 'id ' . $sql, $params);
        // Delete tags.
        \core_tag\privacy\provider::delete_item_tags_select($context, 'mod_datac', 'datac_records', $sql, $params);
        // Delete comments.
        \core_comment\privacy\provider::delete_comments_for_all_users_select($context, 'mod_datac', 'database_entry', $sql, $params);
        // Delete ratings.
        \core_rating\privacy\provider::delete_ratings_select($context, 'mod_datac', 'entry', $sql, $params);
    }
}
