...
/**
 * Search area for mod_datac activity entries.
 *
 * @package    mod_datac
...
class entry extends \core_search\base_mod {
    ...
    public function get_document_recordset($modifiedfrom = 0, \context $context = null) {
        ...
        list ($contextjoin, $contextparams) = $this->get_context_restriction_sql(
                $context, 'datac', 'd', SQL_PARAMS_NAMED);
        if ($contextjoin === null) {
            return null;
        }
    ...
    public function get_document($entry, $options = array()) {
        try {
            $cm = $this->get_cm('datac', $entry->dataid, $entry->course);
            $context = \context_module::instance($cm->id);
        } catch (\dml_missing_record_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_datac ' . $entry->id . ' document, not all required data is available: ' .
                $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        } catch (\dml_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_datac' . $entry->id . ' document: ' . $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
        ...
        // All fields should be already returned as plain text by datac_field_base::get_content_value.
    ...
    public function check_access($id) {
        ...
        $numberofentriesindb = $DB->count_records('datac_records', array('dataid' => $entry->dataid));
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/datac/view.php', array( 'd' => $entry->dataid, 'rid' => $entry->id ));
    }
    ...
    public function get_context_url(\core_search\document $doc) {
        $entry = $this->get_entry($doc->get('itemid'));
        return new \moodle_url('/mod/datac/view.php', array('d' => $entry->dataid));
    }
    ...
    public function attach_files($doc) {
        ...
        $cm = $this->get_cm('datac', $entry->dataid, $doc->get('courseid'));
        $context = \context_module::instance($cm->id);

        // Get the files and attach them.
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'mod_datac', 'content', $entryid, 'filename', false);
    ...
    protected function get_entry($entryid) {
        ...
        if (empty($this->entriesdata[$entryid])) {
            $this->entriesdata[$entryid] = $DB->get_record('datac_records', array( 'id' => $entryid ), '*', MUST_EXIST);
        }
    ...
    protected function get_fields_for_entries($entry) {
        ...
        $sql = "SELECT dc.*, df.name AS fldname,
                  ...
                  FROM {datac_content} dc, {datac_fields} df
        ...
        $template = $DB->get_record_sql('SELECT addtemplate FROM {datac} WHERE id = ?', array($entry->dataid));
        $template = $template->addtemplate;

        // Filtering out the datac_content records having invalid fieldtypes.
    ...
    protected function get_field_class_name($fieldtype) {
        ...
        require_once($CFG->dirroot . '/mod/datac/field/' . $fieldtype . '/field.class.php');
        return 'datac_field_' . $fieldtype;
    }
