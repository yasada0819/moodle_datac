...
$url = new moodle_url('/mod/datac/edit.php');
if ($rid !== 0) {
    $record = $DB->get_record('datac_records', array(
            'id' => $rid,
            'dataid' => $d,
        ), '*', MUST_EXIST);
    $url->param('rid', $rid);
...
if (empty($cm->visible) and !has_capability('moodle/course:viewhiddenactivities', $context)) {
    $strdatabases = get_string("modulenameplural", "datac");
...
if (!has_capability('mod/datac:manageentries', $context)) {
    if ($rid) {
        ...
        if (!datac_user_can_manage_entry($record, $data, $context)) {
            print_error('noaccess','datac');
        }
    } else if (!datac_user_can_add_entry($data, $currentgroup, $groupmode, $context)) {
...
$possiblefields = $DB->get_records('datac_fields', array('dataid'=>$data->id), 'id');
...
if ($rid) {
    $PAGE->navbar->add(get_string('editentry', 'datac'));
}
...
if ($datarecord = data_submitted() and confirm_sesskey()) {
    if ($rid) {
        ...
        $fields = $DB->get_records('datac_fields', array('dataid' => $datarecord->d));

        // Validate the form to ensure that enough data was submitted.
        $processeddata = datac_process_submission($data, $fields, $datarecord);
        ...
        if ($processeddata->validated) {
            ...
            datac_update_record_fields_contents($data, $record, $context, $datarecord, $processeddata);
            core_tag_tag::set_item_tags('mod_datac', 'datac_records', $rid, $context, $tags);

            $viewurl = new moodle_url('/mod/datac/view.php', array(
                'd' => $data->id,
                'rid' => $rid,
            ));
            redirect($viewurl);
        ...
        // Add instance to datac_record.
        if ($processeddata->validated && $recordid = datac_add_record($data, $currentgroup)) {

            // Now populate the fields contents of the new record.
            datac_add_fields_contents_to_new_record($data, $context, $recordid, $fields, $datarecord, $processeddata);

            core_tag_tag::set_item_tags('mod_datac', 'datac_records', $recordid, $context, $tags);
...
groups_print_activity_menu($cm, $CFG->wwwroot.'/mod/datac/edit.php?d='.$data->id);
...
if (!$rid){
    echo $OUTPUT->heading(get_string('newentry','datac'), 3);
}
...
if ($data->addtemplate){
    ...
    foreach ($possiblefields as $eachfield){
        $field = datac_get_field($eachfield, $data);
...
if (!$fields = $DB->get_records('datac_fields', array('dataid'=>$data->id))) {
    print_error('nofieldindatabase', 'datac');
}
