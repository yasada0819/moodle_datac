...
$PAGE->set_url('/mod/datac/export.php', array('d'=>$d));

if (! $data = $DB->get_record('datac', array('id'=>$d))) {
    print_error('wrongdataid', 'datac');
}

if (! $cm = get_coursemodule_from_instance('datac', $data->id, $data->course)) {
...
if(empty($fieldrecords)) {
    if (has_capability('mod/datac:managetemplates', $context)) {
        redirect($CFG->wwwroot.'/mod/datac/field.php?d='.$data->id);
    } else {
...
$mform = new mod_datac_export_form('export.php?d='.$data->id, $fields, $cm, $data);
...
if($mform->is_cancelled()) {
    ...
    $url = new moodle_url('/mod/datac/export.php', array('d' => $d));
...
$exportdata = datac_get_exportdata($data->id, $fields, $selectedfields, $currentgroup, $context,
                                  $exportuser, $exporttime, $exportapproval, $tags);
$count = count($exportdata);
switch ($formdata['exporttype']) {
    case 'csv':
        datac_export_csv($exportdata, $formdata['delimiter_name'], $data->name, $count);
        break;
    case 'xls':
        datac_export_xls($exportdata, $data->name, $count);
        break;
    case 'ods':
        datac_export_ods($exportdata, $data->name, $count);
        break;
}
