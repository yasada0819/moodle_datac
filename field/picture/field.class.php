...
class datac_field_picture extends datac_field_base {
    var $type = 'picture';
    var $previewwidth  = 50;
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        if ($formdata) {
            ...
            if (!$content = $DB->get_record('datac_content', array('fieldid' => $this->field->id, 'recordid' => $recordid))) {
                ...
                $id = $DB->insert_record('datac_content', $content);
                $content = $DB->get_record('datac_content', array('id' => $id));
            }
            file_prepare_draft_area($itemid, $this->context->id, 'mod_datac', 'content', $content->id);
            if (!empty($content->content)) {
                if ($file = $fs->get_file($this->context->id, 'mod_datac', 'content', $content->id, '/', $content->content)) {
        ...
        $str .= '<noscript>';
        if ($file) {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php/', $this->context->id.'/mod_datac/content/'.$content->id.'/'.$file->get_filename());
    ...
    // TODO delete this function and instead subclass datac_field_file - see MDL-16493
    ...
    function display_search_field($value = '') {
        return '<label class="accesshide" for="f_' . $this->field->id . '">' . get_string('fieldname', 'datac') . '</label>' .
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($template == 'listtemplate') {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$this->context->id.'/mod_datac/content/'.$content->id.'/'.'thumb_'.$content->content);
            ...
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$this->context->id.'/mod_datac/content/'.$content->id.'/'.$content->content);
    ...
    function update_field() {
        ...
        $oldfield = $DB->get_record('datac_fields', array('id'=>$this->field->id));
        ...
        if ($oldfield && ($oldfield->param4 != $this->field->param4 || $oldfield->param5 != $this->field->param5)) {
            ...
            if ($contents = $DB->get_records('datac_content', array('fieldid'=>$this->field->id))) {
                $fs = get_file_storage();
                if (count($contents) > 20) {
                    echo $OUTPUT->notification(get_string('resizingimages', 'datac'), 'notifysuccess');
                    echo "\n\n";
                ...
                foreach ($contents as $content) {
                    ...
                    if ($thumbfile = $fs->get_file($this->context->id, 'mod_datac', 'content', $content->id, '/', 'thumb_'.$content->content)) {
                        $thumbfile->delete();
                    }
    ...
    function update_content($recordid, $value, $name='') {
        ...
        $content = $DB->get_record('datac_content', array('fieldid' => $this->field->id, 'recordid' => $recordid));
        if (!$content) {
            $content = (object)array('fieldid' => $this->field->id, 'recordid' => $recordid);
            $content->id = $DB->insert_record('datac_content', $content);
        }
        ...
        switch ($names[2]) {
            case 'file':
                $fs = get_file_storage();
                file_save_draft_area_files($value, $this->context->id, 'mod_datac', 'content', $content->id);
                $usercontext = context_user::instance($USER->id);
                ...
                if (count($files) == 0) {
                    ...
                    if (count($files) > 1) {
                        ...
                        debugging('more then one file found in mod_datac instance {$this->data->id} picture field (field id: {$this->field->id}) area during update data record {$recordid} (content id: {$content->id})', DEBUG_NORMAL);
                    }

                    if ($file->get_imageinfo() === false) {
                        $url = new moodle_url('/mod/datac/edit.php', array('d' => $this->field->dataid));
                        redirect($url, get_string('invalidfiletype', 'error', $file->get_filename()));
                    }
                    $content->content = $file->get_filename();
                    $this->update_thumbnail($content, $file);
                }
                $DB->update_record('datac_content', $content);
