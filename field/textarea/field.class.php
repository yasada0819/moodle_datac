...
class datac_field_textarea extends datac_field_base {
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        if ($formdata) {
            ...
            $text = file_prepare_draft_area($draftitemid, $this->context->id, 'mod_datac', 'content', $content->id, $options, $text);
        } else {
            $draftitemid = file_get_unused_draft_itemid();
            $format = FORMAT_HTML;
        }
    ...
    function update_content($recordid, $value, $name='') {
        ...
        if (!empty($content->content)) {
            ...
            $options = $this->get_options();
            $content->content = file_save_draft_area_files($draftitemid, $this->context->id, 'mod_datac', 'content', $content->id, $options, $content->content);
        }
        $rv = $DB->update_record('datac_content', $content);
        return $rv;
    }
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($content = $DB->get_record('datac_content', array('fieldid' => $this->field->id, 'recordid' => $recordid))) {
            if (isset($content->content)) {
                ...
                $options->para = false;
                $str = file_rewrite_pluginfile_urls($content->content, 'pluginfile.php', $this->context->id, 'mod_datac', 'content', $content->id, $this->get_options());
