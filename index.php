...
$PAGE->set_url('/mod/datac/index.php', array('id'=>$id));
...
$event = \mod_datac\event\course_module_instance_list_viewed::create($params);
$event->add_record_snapshot('course', $course);
$event->trigger();

$strname = get_string('name');
$strdata = get_string('modulename','datac');
$strdataplural  = get_string('modulenameplural','datac');

$PAGE->navbar->add($strdata, new moodle_url('/mod/datac/index.php', array('id'=>$course->id)));
$PAGE->set_title($strdata);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();
echo $OUTPUT->heading($strdataplural, 2);

if (! $datas = get_all_instances_in_course("datac", $course)) {
...
$strentries = get_string('entries', 'datac');
$strnumnotapproved = get_string('numnotapproved', 'datac');
...
foreach ($datas as $data) {
    ...
    $numrecords = $DB->count_records_sql('SELECT COUNT(r.id) FROM {datac_records} r WHERE r.dataid =?', array($data->id));

    if ($data->approval == 1) {
        $numunapprovedrecords = $DB->count_records_sql('SELECT COUNT(r.id) FROM {datac_records} r WHERE r.dataid =? AND r.approved <> 1', array($data->id));
    } else {
    ...
    if ($rss && $data->rssarticles > 0) {
        $rsslink = rss_get_link($context->id, $USER->id, 'mod_datac', $data->id, 'RSS');
    }
