...
define ('DATA_CAP_EXPORT', 'mod/datac:viewalluserpresets');

define('DATA_PRESET_COMPONENT', 'mod_datac');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class datac_field_base {     // Base class for Database Field Types (see field/*/field.class.php)
    ...
    function __construct($field=0, $data=0, $cm=0) {   // Field or data or both, each can be id or object
        ...
        if (empty($field) && empty($data)) {
            print_error('missingfield', 'datac');
        }

        if (!empty($field)) {
            if (is_object($field)) {
                $this->field = $field;  // Programmer knows what they are doing, we hope
            } else if (!$this->field = $DB->get_record('datac_fields', array('id'=>$field))) {
                print_error('invalidfieldid', 'datac');
            }
            if (empty($data)) {
                if (!$this->data = $DB->get_record('datac', array('id'=>$this->field->dataid))) {
                    print_error('invalidid', 'datac');
                }
            }
        }
        ...
        if (empty($this->data)) {         // We need to define this properly
            if (!empty($data)) {
                ...
                } else if (!$this->data = $DB->get_record('datac', array('id'=>$data))) {
                    print_error('invalidid', 'datac');
                }
            } else {                      // No way to define it!
                print_error('missingdata', 'datac');
            }
        }
        ...
        if ($cm) {
            ...
            $this->cm = get_coursemodule_from_instance('datac', $this->data->id);
        }
    ...
    function insert_field() {
        ...
        $this->field->id = $DB->insert_record('datac_fields',$this->field);

        // Trigger an event for creating this field.
        $event = \mod_datac\event\field_created::create(array(
    ...
    function update_field() {
        ...
        $DB->update_record('datac_fields', $this->field);

        // Trigger an event for updating this field.
        $event = \mod_datac\event\field_updated::create(array(
            'objectid' => $this->field->id,
            'context' => $this->context,
            'other' => array(
                'fieldname' => $this->field->name,
    ...
    function delete_field() {
        ...
        if (!empty($this->field->id)) {
            ...
            $field = $DB->get_record('datac_fields', array('id' => $this->field->id));

            $this->delete_content();
            $DB->delete_records('datac_fields', array('id'=>$this->field->id));

            // Trigger an event for deleting this field.
            $event = \mod_datac\event\field_deleted::create(array(
                'objectid' => $this->field->id,
                'context' => $this->context,
                'other' => array(
                    'fieldname' => $this->field->name,
            ...
            $event->add_record_snapshot('datac_fields', $field);
            $event->trigger();
        }
    ...
    function display_add_field($recordid=0, $formdata=null) {
        ...
        if ($formdata) {
            ...
            $content = $DB->get_field('datac_content', 'content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid));
        } else {
            $content = '';
        }
        ...
        $str .= '</label><input class="basefieldinput form-control d-inline mod-datac-input" ' .
    ...
    function display_edit_field() {
        ...
        echo '<form id="editfield" action="'.$CFG->wwwroot.'/mod/datac/field.php" method="post">'."\n";
        ...
        require_once($CFG->dirroot.'/mod/datac/field/'.$this->type.'/mod.html');
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($content = $DB->get_record('datac_content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))) {
    ...
    /**
     * Update the content of one data field in the datac_content table
     * @global object
     * @param int $recordid
     * @param mixed $value
     * @param string $name
    ...
    function update_content($recordid, $value, $name=''){
        ...
        if ($oldcontent = $DB->get_record('datac_content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))) {
            $content->id = $oldcontent->id;
            return $DB->update_record('datac_content', $content);
        } else {
            return $DB->insert_record('datac_content', $content);
        }
    }
    ...
    function delete_content($recordid=0) {
        ...
        $rs = $DB->get_recordset('datac_content', $conditions);
        if ($rs->valid()) {
            $fs = get_file_storage();
            foreach ($rs as $content) {
                $fs->delete_area_files($this->context->id, 'mod_datac', 'content', $content->id);
            }
        }
        $rs->close();

        return $DB->delete_records('datac_content', $conditions);
    }
     ...
     */
    function name() {
        return get_string('fieldtypelabel', "datacfield_$this->type");
    }
    ...
    function image() {
        ...
        $link = new moodle_url('/mod/datac/field.php', $params);
        $str = '<a href="'.$link->out().'">';
        $str .= $OUTPUT->pix_icon('field/' . $this->type, $this->type, 'datac');
        $str .= '</a>';
        return $str;
    }
 ...
 * @return bool|string
 */
function datac_generate_default_template(&$data, $template, $recordid=0, $form=false, $update=true) {
    global $DB;
    ...
    if ($fields = $DB->get_records('datac_fields', array('dataid'=>$data->id), 'id')) {

        $table = new html_table();
        $table->attributes['class'] = 'mod-datac-default-template ##approvalstatus##';
        $table->colclasses = array('template-field', 'template-token');
        $table->data = array();
        foreach ($fields as $field) {
            if ($form) {   // Print forms instead of data
                $fieldobj = datac_get_field($field, $data);
        ...
        if ($template == 'listtemplate') {
            ...
            $row = new html_table_row(array(get_string('authorfirstname', 'datac').': ', '##firstname##'));
            $row->attributes['class'] = 'searchcontrols';
            $table->data[] = $row;
            $row = new html_table_row(array(get_string('authorlastname', 'datac').': ', '##lastname##'));
            $row->attributes['class'] = 'searchcontrols';
            $table->data[] = $row;
        }
        ...
        if ($update) {
            ...
            $DB->update_record('datac', $newdata);
            $data->{$template} = $str;
        }
 ...
 * @return string
 */
function datac_generate_tag_form($recordid = false, $selected = []) {
    global $CFG, $DB, $OUTPUT, $PAGE;

    $tagtypestoshow = \core_tag_area::get_showstandard('mod_datac', 'datac_records');
    ...
    $tagcollid = \core_tag_area::get_collection('mod_datac', 'datac_records');
    $tags = [];
    $selectedtags = [];
    ...
    if ($recordid) {
        $selectedtags += core_tag_tag::get_item_tags_array('mod_datac', 'datac_records', $recordid);
    }
    ...
    if (has_capability('moodle/tag:manage', context_system::instance()) && $showstandard) {
        $url = new moodle_url('/tag/manage.php', array('tc' => core_tag_area::get_collection('mod_datac',
            'datac_records')));
 ...
 * @return bool
 */
function datac_replace_field_in_templates($data, $searchfieldname, $newfieldname) {
    global $DB;
    ...
    return $DB->update_record('datac', $newdata);
}
 ...
 * @param string $newfieldname
 */
function datac_append_new_field_to_templates($data, $newfieldname) {
    global $DB;
 ...
 * @return object|bool
 */
function datac_get_field_from_name($name, $data){
    global $DB;

    $field = $DB->get_record('datac_fields', array('name'=>$name, 'dataid'=>$data->id));

    if ($field) {
        return datac_get_field($field, $data);
    } else {
        return false;
    }
}
 ...
 * @return bool|object
 */
function datac_get_field_from_id($fieldid, $data){
    global $DB;

    $field = $DB->get_record('datac_fields', array('id'=>$fieldid, 'dataid'=>$data->id));
 ...
 * @return object
 */
function datac_get_field_new($type, $data) {
    global $CFG;

    require_once($CFG->dirroot.'/mod/datac/field/'.$type.'/field.class.php');
    $newfield = 'datac_field_'.$type;
 ...
 * @return object
 */
function datac_get_field($field, $data, $cm=null) {
    global $CFG;

    if ($field) {
        require_once('field/'.$field->type.'/field.class.php');
        $newfield = 'datac_field_'.$field->type;
...
function datac_isowner($record) {
    ...
    if (!is_object($record)) {
        if (!$record = $DB->get_record('datac_records', array('id'=>$record))) {
            return false;
        }
    }
 ...
 * @return bool
 */
function datac_atmaxentries($data){
    if (!$data->maxentries){
        return false;

    } else {
        return (datac_numentries($data) >= $data->maxentries);
    }
}
 ...
 * @return int
 */
function datac_numentries($data, $userid=null) {
    global $USER, $DB;
    if ($userid === null) {
        $userid = $USER->id;
    }
    $sql = 'SELECT COUNT(*) FROM {datac_records} WHERE dataid=? AND userid=?';
 ...
 * @return bool
 */
function datac_add_record($data, $groupid=0, $opt=array()){
    global $USER, $DB;

    $cm = get_coursemodule_from_instance('datac', $data->id);
    ...
    if(empty($opt)){
        ...
        if (has_capability('mod/datac:approve', $context)) {
            $record->approved = 1;
        } else {
            $record->approved = 0;
        }
        ...
        $record = new stdClass();
        if(isset($opt['userid'])){
            $record->userid = $opt['userid'];
        }else{
            $record->userid = $USER->id;
        }
        $record->dataid = $data->id;
        $record->groupid = $groupid;
        if(isset($opt['timecreated']) && isset($opt['timemodified'])){
            $record->timecreated = $opt['timecreated'];
            $record->timemodified = $opt['timemodified'];
        }else if(isset($opt['timecreated'])){
            $record->timecreated = $record->timemodified = $opt['timecreated'];
        }else if(isset($opt['timemodified'])){
            $record->timecreated = $record->timemodified = $opt['timemodified'];
        }else{
            $record->timecreated = $record->timemodified = time();
        }
        if (has_capability('mod/datac:approve', $context, $record->userid)) {
            $record->approved = 1;
        } else {
            $record->approved = 0;
        }
    ...
    $record->id = $DB->insert_record('datac_records', $record);

    // Trigger an event for creating this record.
    $event = \mod_datac\event\record_created::create(array(
        'objectid' => $record->id,
        'context' => $context,
        'other' => array(
            'dataid' => $data->id
    ...
    datac_update_completion_state($data, $course, $cm);
 ...
 * @return bool
 */
function datac_tags_check($dataid, $template) {
    global $DB, $OUTPUT;

    // first get all the possible tags
    $fields = $DB->get_records('datac_fields', array('dataid'=>$dataid));
    ...
    foreach ($fields as $field){
        ...
        if (preg_match_all($pattern, $template, $dummy)>1){
            $tagsok = false;
            echo $OUTPUT->notification('[['.$field->name.']] - '.get_string('multipletags','datac'));
        }
    }
    // else return true
    return $tagsok;
 ...
 *
 * @param stdClass $data
 * @param mod_datac_mod_form $mform
 * @return int intance id
 */
function datac_add_instance($data, $mform = null) {
    global $DB, $CFG;
    ...
    $data->id = $DB->insert_record('datac', $data);

    // Add calendar events if necessary.
    datac_set_events($data);
    if (!empty($data->completionexpected)) {
        \core_completion\api::update_completion_date_event($data->coursemodule, 'datac', $data->id, $data->completionexpected);
    }

    datac_grade_item_update($data);
 ...
 * @return bool
 */
function datac_update_instance($data) {
    global $DB, $CFG;
    require_once($CFG->dirroot.'/mod/datac/locallib.php');
    ...
    $DB->update_record('datac', $data);

    // Add calendar events if necessary.
    datac_set_events($data);
    $completionexpected = (!empty($data->completionexpected)) ? $data->completionexpected : null;
    \core_completion\api::update_completion_date_event($data->coursemodule, 'datac', $data->id, $completionexpected);
 ...
 * @return bool
 */
function datac_delete_instance($id) {    // takes the dataid
    global $DB, $CFG;

    if (!$data = $DB->get_record('datac', array('id'=>$id))) {
        return false;
    }
...
/// Delete all the associated information
    ...
    // files
    $fs = get_file_storage();
    $fs->delete_area_files($context->id, 'mod_datac');

    // get all the records in this data
    $sql = "SELECT r.id
              FROM {datac_records} r
             WHERE r.dataid = ?";

    $DB->delete_records_select('datac_content', "recordid IN ($sql)", array($id));

    // delete all the records and fields
    $DB->delete_records('datac_records', array('dataid'=>$id));
    $DB->delete_records('datac_fields', array('dataid'=>$id));

    // Remove old calendar events.
    $events = $DB->get_records('event', array('modulename' => 'datac', 'instance' => $id));
    ...
    $result = $DB->delete_records('datac', array('id'=>$id));

    // cleanup gradebook
    datac_grade_item_delete($data);
 ...
 * @return object|null
 */
function datac_user_outline($course, $user, $mod, $data) {
    global $DB, $CFG;
    require_once("$CFG->libdir/gradelib.php");

    $grades = grade_get_grades($course->id, 'mod', 'datac', $data->id, $user->id);
    ...
    if ($countrecords = $DB->count_records('datac_records', array('dataid'=>$data->id, 'userid'=>$user->id))) {
        $result = new stdClass();
        $result->info = get_string('numrecords', 'datac', $countrecords);
        $lastrecord   = $DB->get_record_sql('SELECT id,timemodified FROM {datac_records}
 ...
 * @param object $data
 */
function datac_user_complete($course, $user, $mod, $data) {
    ...
    if ($records = $DB->get_records('datac_records', array('dataid'=>$data->id,'userid'=>$user->id), 'timemodified DESC')) {
        datac_print_template('singletemplate', $records, $data);
    }
}
 ...
 */
function datac_get_user_grades($data, $userid=0) {
    global $CFG;

    require_once($CFG->dirroot.'/rating/lib.php');

    $ratingoptions = new stdClass;
    $ratingoptions->component = 'mod_datac';
    $ratingoptions->ratingarea = 'entry';
    $ratingoptions->modulename = 'datac';
    $ratingoptions->moduleid   = $data->id;

    $ratingoptions->userid = $userid;
    $ratingoptions->aggregationmethod = $data->assessed;
    $ratingoptions->scaleid = $data->scale;
    $ratingoptions->itemtable = 'datac_records';
 ...
 */
function datac_update_grades($data, $userid=0, $nullifnone=true) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');
    ...
    } else if ($grades = datac_get_user_grades($data, $userid)) {
        datac_grade_item_update($data, $grades);

    } else if ($userid and $nullifnone) {
        $grade = new stdClass();
        $grade->userid   = $userid;
        $grade->rawgrade = NULL;
        datac_grade_item_update($data, $grade);
 ...
 */
function datac_grade_item_update($data, $grades=NULL) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');
    ...
    return grade_update('mod/data', $data->course, 'mod', 'datac', $data->id, 0, $grades, $params);
}
 ...
 * @return object grade_item
 */
function datac_grade_item_delete($data) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    return grade_update('mod/data', $data->course, 'mod', 'datac', $data->id, 0, NULL, array('deleted'=>1));
}
 ...
 * @return mixed
 */
function datac_print_template($template, $records, $data, $search='', $page=0, $return=false, moodle_url $jumpurl=null) {
    global $CFG, $DB, $OUTPUT;
    ...
    if (empty($fields)) {
        $fieldrecords = $DB->get_records('datac_fields', array('dataid'=>$data->id));
        foreach ($fieldrecords as $fieldrecord) {
            $fields[]= datac_get_field($fieldrecord, $data);
        }
    }
    ...
    if (!$jumpurl) {
        $jumpurl = new moodle_url('/mod/datac/view.php', array('d' => $data->id));
    }
    ...
    // Then we generate strings to replace for normal tags
        ...
        $canmanageentries = has_capability('mod/datac:manageentries', $context);

    // Replacing special tags (##Edit##, ##Delete##, ##More##)
        $patterns[]='##edit##';
        $patterns[]='##delete##';
        if (datac_user_can_manage_entry($record, $data, $context)) {
            $replacement[] = '<a href="'.$CFG->wwwroot.'/mod/datac/edit.php?d='
                             .$data->id.'&amp;rid='.$record->id.'&amp;sesskey='.sesskey().'">' .
                             $OUTPUT->pix_icon('t/edit', get_string('edit')) . '</a>';
            $replacement[] = '<a href="'.$CFG->wwwroot.'/mod/datac/view.php?d='
        ...
        $moreurl = $CFG->wwwroot . '/mod/datac/view.php?d=' . $data->id . '&amp;rid=' . $record->id;
        if ($search) {
            $moreurl .= '&amp;filter=1';
        }
        $patterns[]='##more##';
        $replacement[] = '<a href="'.$moreurl.'">' . $OUTPUT->pix_icon('t/preview', get_string('more', 'datac')) . '</a>';
        ...
        if (!empty($CFG->enableportfolios) && ($template == 'singletemplate' || $template == 'listtemplate')
            && ((has_capability('mod/datac:exportentry', $context)
                || (datac_isowner($record->id) && has_capability('mod/datac:exportownentry', $context))))) {
            require_once($CFG->libdir . '/portfoliolib.php');
            $button = new portfolio_add_button();
            $button->set_callback_options('datac_portfolio_caller', array('id' => $cm->id, 'recordid' => $record->id), 'mod_datac');
            list($formats, $files) = datac_portfolio_caller::formats($fields, $record);
        ...
        $patterns[]='##approve##';
        if (has_capability('mod/datac:approve', $context) && ($data->approval) && (!$record->approved)) {
            $approveurl = new moodle_url($jumpurl, array('approve' => $record->id));
            $approveicon = new pix_icon('t/approve', get_string('approve', 'datac'), '', array('class' => 'iconsmall'));
        ...
        if (has_capability('mod/datac:approve', $context) && ($data->approval) && ($record->approved)) {
            ...
            $disapproveicon = new pix_icon('t/block', get_string('disapprove', 'datac'), '', array('class' => 'iconsmall'));
        ...
        if (!$data->approval) {
            ...
            $replacement[] = get_string('approved', 'datac');
        } else {
            $replacement[] = get_string('notapproved', 'datac');
        }
        ...
        if (core_tag_tag::is_enabled('mod_datac', 'datac_records')) {
            ...
            $replacement[] = $OUTPUT->tag_list(
                core_tag_tag::get_item_tags('mod_datac', 'datac_records', $record->id), '', 'data-tags');
        }
        ...
        if ($return) {
            ...
            if ($template == 'singletemplate') {    //prints ratings options
                datac_print_ratings($data, $record);
            }
 ...
 */
function datac_rating_permissions($contextid, $component, $ratingarea) {
    $context = context::instance_by_id($contextid, MUST_EXIST);
    if ($component != 'mod_datac' || $ratingarea != 'entry') {
        return null;
    }
    return array(
        'view'    => has_capability('mod/datac:viewrating',$context),
        'viewany' => has_capability('mod/datac:viewanyrating',$context),
        'viewall' => has_capability('mod/datac:viewallratings',$context),
        'rate'    => has_capability('mod/datac:rate',$context)
    );
}
 ...
 */
function datac_rating_validate($params) {
    global $DB, $USER;

    // Check the component is mod_datac
    if ($params['component'] != 'mod_datac') {
    ...
    $cm = get_coursemodule_from_instance('datac', $info->dataid, $course->id, false, MUST_EXIST);
    $context = context_module::instance($cm->id);
 ...
 *            component => The component for this module - should always be mod_datac [required]
 ...
 */
function mod_datac_rating_can_see_item_ratings($params) {
    global $DB;

    // Check the component is mod_datac.
    if (!isset($params['component']) || $params['component'] != 'mod_datac') {
        throw new rating_exception('invalidcomponent');
    }
 ...
 * @return void
 */
function datac_print_preference_form($data, $perpage, $search, $sort='', $order='ASC', $search_array = '', $advanced = 0, $mode= ''){
    ...
    echo '<label for="pref_perpage">'.get_string('pagesize','datac').'</label> ';
    ...
    if ($fields = $DB->get_records('datac_fields', array('dataid'=>$data->id), 'name')) {
        echo '<optgroup label="'.get_string('fields', 'datac').'">';
        foreach ($fields as $field) {
            if ($field->id == $sort) {
    ...
    $options = array();
    $options[DATA_TIMEADDED]    = get_string('timeadded', 'datac');
    $options[DATA_TIMEMODIFIED] = get_string('timemodified', 'datac');
    $options[DATA_FIRSTNAME]    = get_string('authorfirstname', 'datac');
    $options[DATA_LASTNAME]     = get_string('authorlastname', 'datac');
    if ($data->approval and has_capability('mod/datac:approve', $context)) {
        $options[DATA_APPROVED] = get_string('approved', 'datac');
    }
    echo '<optgroup label="'.get_string('other', 'datac').'">';
    ...
    if ($order == 'ASC') {
        echo '<option value="ASC" selected="selected">'.get_string('ascending','datac').'</option>';
    } else {
        echo '<option value="ASC">'.get_string('ascending','datac').'</option>';
    }
    if ($order == 'DESC') {
        echo '<option value="DESC" selected="selected">'.get_string('descending','datac').'</option>';
    } else {
        echo '<option value="DESC">'.get_string('descending','datac').'</option>';
    }
    echo '</select>';
    ...
    $PAGE->requires->js('/mod/datac/data.js');
    ...
    echo '&nbsp;<input type="checkbox" id="advancedcheckbox" name="advanced" value="1" ' . $checked . ' ' .
         ...
         '<label for="advancedcheckbox">' . get_string('advancedsearch', 'datac') . '</label>';
    echo '&nbsp;<input type="submit" class="btn btn-secondary" value="' . get_string('savesettings', 'datac') . '" />';

    echo '<br />';
    echo '<div class="' . $advancedsearchclass . '" id="datac_adv_form">';
    ...
    if(empty($data->asearchtemplate)) {
        datac_generate_default_template($data, 'asearchtemplate');
    }
    ...
    foreach ($fields as $field) {
        ...
        $searchfield = datac_get_field_from_id($field->field->id, $data);
    ...
    $patterns[]    = '/##firstname##/';
    $replacement[] = '<label class="accesshide" for="u_fn">' . get_string('authorfirstname', 'datac') . '</label>' .
                     '<input type="text" class="form-control" size="16" id="u_fn" name="u_fn" value="' . s($fn) . '" />';
    $patterns[]    = '/##lastname##/';
    $replacement[] = '<label class="accesshide" for="u_ln">' . get_string('authorlastname', 'datac') . '</label>' .
    ...
    if (core_tag_tag::is_enabled('mod_datac', 'datac_records')) {
        ...
        $replacement[] = datac_generate_tag_form(false, $selectedtags);
    }
    ...
    echo '<tr><td colspan="4"><br/>' .
         '<input type="submit" class="btn btn-primary m-r-1" value="' . get_string('savesettings', 'datac') . '" />' .
         '<input type="submit" class="btn btn-secondary" name="resetadv" value="' . get_string('resetsettings', 'datac') . '" />' .
         '</td></tr>';
    echo '</table>';
    echo '</div>';
    echo '</div>';
 ...
 */
function datac_print_ratings($data, $record) {
    global $OUTPUT;
 ...
 * @return array
 */
function datac_get_view_actions() {
    return array('view');
}
 ...
 * @return array
 */
function datac_get_post_actions() {
 ...
 * @return bool
 */
function datac_fieldname_exists($name, $dataid, $fieldid = 0) {
    global $DB;
    ...
    if ($fieldid) {
        ...
        return $DB->record_exists_sql("SELECT * FROM {datac_fields} df
 ...
 */
function datac_convert_arrays_to_strings(&$fieldinput) {
 ...
 */
function datac_convert_to_roles($data, $teacherroles=array(), $studentroles=array(), $cmid=NULL) {
    global $CFG, $DB, $OUTPUT;
    ...
    if (empty($cmid)) {
        ...
        if (!$cm = get_coursemodule_from_instance('datac', $data->id)) {
    ...
    switch ($data->participants) {
        case 1:
            foreach ($studentroles as $studentrole) {
                assign_capability('mod/datac:writeentry', CAP_PREVENT, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('mod/datac:writeentry', CAP_ALLOW, $teacherrole->id, $context->id);
            }
            break;
        case 3:
            foreach ($studentroles as $studentrole) {
                assign_capability('mod/datac:writeentry', CAP_ALLOW, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('mod/datac:writeentry', CAP_ALLOW, $teacherrole->id, $context->id);
            }
    ...
    switch ($data->assessed) {
        case 0:
            foreach ($studentroles as $studentrole) {
                assign_capability('mod/datac:rate', CAP_PREVENT, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('mod/datac:rate', CAP_PREVENT, $teacherrole->id, $context->id);
            }
            break;
        case 1:
            foreach ($studentroles as $studentrole) {
                assign_capability('mod/datac:rate', CAP_ALLOW, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('mod/datac:rate', CAP_ALLOW, $teacherrole->id, $context->id);
            }
            break;
        case 2:
            foreach ($studentroles as $studentrole) {
    ...
    switch ($data->assesspublic) {
        case 0:
            foreach ($studentroles as $studentrole) {
                assign_capability('mod/datac:viewrating', CAP_PREVENT, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('mod/datac:viewrating', CAP_ALLOW, $teacherrole->id, $context->id);
            }
            break;
        case 1:
            foreach ($studentroles as $studentrole) {
                assign_capability('mod/datac:viewrating', CAP_ALLOW, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('mod/datac:viewrating', CAP_ALLOW, $teacherrole->id, $context->id);
            }
 ...
 * @return string
 */
function datac_preset_name($shortname, $path) {

    // We are looking inside the preset itself as a first choice, but also in normal data directory
    $string = get_string('modulename', 'datacpreset_'.$shortname);
 ...
 * @return array
 */
function datac_get_available_presets($context) {
    global $CFG, $USER;

    $presets = array();

    // First load the ratings sub plugins that exist within the modules preset dir
    if ($dirs = core_component::get_plugin_list('datacpreset')) {
        foreach ($dirs as $dir=>$fulldir) {
            if (is_directory_a_preset_c($fulldir)) {
                $preset = new stdClass();
                $preset->path = $fulldir;
                $preset->userid = 0;
                $preset->shortname = $dir;
                $preset->name = datac_preset_name($dir, $fulldir);
                if (file_exists($fulldir.'/screenshot.jpg')) {
                    $preset->screenshot = $CFG->wwwroot.'/mod/datac/preset/'.$dir.'/screenshot.jpg';
                } else if (file_exists($fulldir.'/screenshot.png')) {
                    $preset->screenshot = $CFG->wwwroot.'/mod/datac/preset/'.$dir.'/screenshot.png';
                } else if (file_exists($fulldir.'/screenshot.gif')) {
                    $preset->screenshot = $CFG->wwwroot.'/mod/datac/preset/'.$dir.'/screenshot.gif';
                }
                $presets[] = $preset;
            }
        }
    ...
    $presets = datac_get_available_site_presets($context, $presets);
    return $presets;
}
 ...
 */
function datac_get_available_site_presets($context, array $presets=array()) {
    global $USER;

    $fs = get_file_storage();
    $files = $fs->get_area_files(DATA_PRESET_CONTEXT, DATA_PRESET_COMPONENT, DATA_PRESET_FILEAREA);
    $canviewall = has_capability('mod/datac:viewalluserpresets', $context);
    if (empty($files)) {
        return $presets;
    }
 ...
 * @return bool
 */
function datac_delete_site_preset($name) {
    $fs = get_file_storage();
 ...
 */
function datac_print_header($course, $cm, $data, $currenttab='') {
 ...
 * @return bool
 */
function datac_user_can_add_entry($data, $currentgroup, $groupmode, $context = null) {
    global $USER;

    if (empty($context)) {
        $cm = get_coursemodule_from_instance('datac', $data->id, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
    }

    if (has_capability('mod/datac:manageentries', $context)) {
        // no entry limits apply if user can manage

    } else if (!has_capability('mod/datac:writeentry', $context)) {
        return false;

    } else if (datac_atmaxentries($data)) {
        return false;
    } else if (datac_in_readonly_period($data)) {
 ...
 * datac_in_readonly_period() result, ownership (determined by datac_isowner()) and manageapproved setting.
 * @param mixed $record record object or id
 * @param object $data data object
 * @param object $context context object
 * @return bool returns true if the user is allowd to edit the entry, false otherwise
 */
function datac_user_can_manage_entry($record, $data, $context) {
    global $DB;
    ...
    $readonly = datac_in_readonly_period($data);

    if (!$readonly) {
        // Get record object from db if just id given like in datac_isowner.
        // ...done before calling datac_isowner() to avoid querying db twice.
        if (!is_object($record)) {
            if (!$record = $DB->get_record('datac_records', array('id' => $record))) {
                return false;
            }
        }
        if (datac_isowner($record)) {
 ...
 */
function datac_in_readonly_period($data) {
    $now = time();
 ...
 * @return bool
 */
function is_directory_a_preset_c($directory) {
 ...
 */
abstract class datac_preset_importer {
     ...
     */
    public function datac_preset_get_file_contents(&$filestorage, &$fileobj, $dir, $filename) {
    ...
    public function get_preset_settings() {
        ...
        $fs = $fileobj = null;
        if (!is_directory_a_preset_c($this->directory)) {
            ...
            if (empty($fileobj)) {
                print_error('invalidpreset', 'datac', '', $this->directory);
            }
        }
        ...
        $result->currentfields = $DB->get_records('datac_fields', array('dataid'=>$this->module->id));
        ...
        /* Grab XML */
        $presetxml = $this->datac_preset_get_file_contents($fs, $fileobj, $this->directory,'preset.xml');
        ...
        $result->settings->singletemplate     = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"singletemplate.html");
        $result->settings->listtemplate       = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"listtemplate.html");
        $result->settings->listtemplateheader = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"listtemplateheader.html");
        $result->settings->listtemplatefooter = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"listtemplatefooter.html");
        $result->settings->addtemplate        = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"addtemplate.html");
        $result->settings->rsstemplate        = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"rsstemplate.html");
        $result->settings->rsstitletemplate   = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"rsstitletemplate.html");
        $result->settings->csstemplate        = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"csstemplate.css");
        $result->settings->jstemplate         = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"jstemplate.js");
        $result->settings->asearchtemplate    = $this->datac_preset_get_file_contents($fs, $fileobj,$this->directory,"asearchtemplate.html");
    ...
    function import($overwritesettings) {
        ...
        if (!empty($newfields)) {
            ...
            foreach ($newfields as $nid => $newfield) {
                ...
                if (array_key_exists($cid, $preservedfields)){
                    print_error('notinjectivemap', 'datac');
                }
            ...
            foreach ($newfields as $nid => $newfield) {
                ...
                if ($cid != -1 and isset($currentfields[$cid])) {
                    $fieldobject = datac_get_field_from_id($currentfields[$cid]->id, $this->module);
                    ...
                    $classname = 'datac_field_'.$newfield->type;
        ...
        if (!empty($preservedfields)) {
            foreach ($currentfields as $cid => $currentfield) {
                if (!array_key_exists($cid, $preservedfields)) {
                    ...
                    $DB->delete_records('datac_content', array('fieldid'=>$id));
                    $DB->delete_records('datac_fields', array('id'=>$id));
                }
            }
        }
        ...
        if (!empty($settings->defaultsort)) {
            if (is_numeric($settings->defaultsort)) {
                ...
                $settings->defaultsort = (int)$DB->get_field('datac_fields', 'id', array('dataid'=>$this->module->id, 'name'=>$settings->defaultsort));
            }
        } else {
            $settings->defaultsort = 0;
        }
        ...
        datac_update_instance($this->module);
 ...
 */
class datac_preset_upload_importer extends datac_preset_importer {
 ...
 */
class datac_preset_existing_importer extends datac_preset_importer {
    protected $userid;
    public function __construct($course, $cm, $module, $fullname) {
        global $USER;
        list($userid, $shortname) = explode('/', $fullname, 2);
        $context = context_module::instance($cm->id);
        if ($userid && ($userid != $USER->id) && !has_capability('mod/datac:manageuserpresets', $context) && !has_capability('mod/datac:viewalluserpresets', $context)) {
           throw new coding_exception('Invalid preset provided');
        }

        $this->userid = $userid;
        $filepath = datac_preset_path($course, $userid, $shortname);
        parent::__construct($course, $cm, $module, $filepath);
    }
 ...
 * @return string
 */
function datac_preset_path($course, $userid, $shortname) {
    global $USER, $CFG;
    ...
    $path = null;
    if ($userid > 0 && ($userid == $USER->id || has_capability('mod/datac:viewalluserpresets', $context))) {
        $path = $CFG->dataroot.'/data/preset/'.$userid.'/'.$shortname;
    } else if ($userid == 0) {
        $path = $CFG->dirroot.'/mod/datac/preset/'.$shortname;
    } else if ($userid < 0) {
 ...
 */
function datac_reset_course_form_definition(&$mform) {
    $mform->addElement('header', 'dataheader', get_string('modulenameplural', 'datac'));
    $mform->addElement('checkbox', 'reset_datac', get_string('deleteallentries','datac'));

    $mform->addElement('checkbox', 'reset_datac_notenrolled', get_string('deletenotenrolled', 'datac'));
    $mform->disabledIf('reset_datac_notenrolled', 'reset_datac', 'checked');

    $mform->addElement('checkbox', 'reset_datac_ratings', get_string('deleteallratings'));
    $mform->disabledIf('reset_datac_ratings', 'reset_datac', 'checked');

    $mform->addElement('checkbox', 'reset_datac_comments', get_string('deleteallcomments'));
    $mform->disabledIf('reset_datac_comments', 'reset_datac', 'checked');

    $mform->addElement('checkbox', 'reset_datac_tags', get_string('removealldatatags', 'datac'));
    $mform->disabledIf('reset_datac_tags', 'reset_datac', 'checked');
}
 ...
 * @return array
 */
function datac_reset_course_form_defaults($course) {
    return array('reset_datac'=>0, 'reset_datac_ratings'=>1, 'reset_datac_comments'=>1, 'reset_datac_notenrolled'=>0);
}
 ...
 */
function datac_reset_gradebook($courseid, $type='') {
    global $CFG, $DB;

    $sql = "SELECT d.*, cm.idnumber as cmidnumber, d.course as courseid
              FROM {datac} d, {course_modules} cm, {modules} m
             WHERE m.name='datac' AND m.id=cm.module AND cm.instance=d.id AND d.course=?";

    if ($datas = $DB->get_records_sql($sql, array($courseid))) {
        foreach ($datas as $data) {
            datac_grade_item_update($data, 'reset');
        }
    }
}
 ...
 */
function datac_reset_userdata($data) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/filelib.php');
    require_once($CFG->dirroot.'/rating/lib.php');

    $componentstr = get_string('modulenameplural', 'datac');
    $status = array();

    $allrecordssql = "SELECT r.id
                        FROM {datac_records} r
                             INNER JOIN {datac} d ON r.dataid = d.id
                       WHERE d.course = ?";
    ...
    $ratingdeloptions->component = 'mod_datac';
    ...
    if (!empty($data->reset_data)) {
        ...
        $DB->delete_records_select('datac_content', "recordid IN ($allrecordssql)", array($data->courseid));
        $DB->delete_records_select('datac_records', "dataid IN ($alldatassql)", array($data->courseid));

        if ($datas = $DB->get_records_sql($alldatassql, array($data->courseid))) {
            foreach ($datas as $dataid=>$unused) {
                if (!$cm = get_coursemodule_from_instance('datac', $dataid)) {
                    continue;
                }
                $datacontext = context_module::instance($cm->id);

                // Delete any files that may exist.
                $fs->delete_area_files($datacontext->id, 'mod_datac', 'content');

                $ratingdeloptions->contextid = $datacontext->id;
                $rm->delete_ratings($ratingdeloptions);

                core_tag_tag::delete_instances('mod_datac', null, $datacontext->id);
            }
        }

        if (empty($data->reset_gradebook_grades)) {
            // remove all grades from gradebook
            datac_reset_gradebook($data->courseid);
        }
        $status[] = array('component'=>$componentstr, 'item'=>get_string('deleteallentries', 'datac'), 'error'=>false);
    }

    // remove entries by users not enrolled into course
    if (!empty($data->reset_datac_notenrolled)) {
        ...
        foreach ($rs as $record) {
            if (array_key_exists($record->userid, $notenrolled) or !$record->userexists or $record->userdeleted
              or !is_enrolled($course_context, $record->userid)) {
                //delete ratings
                if (!$cm = get_coursemodule_from_instance('datac', $record->dataid)) {
                    continue;
                }
                $datacontext = context_module::instance($cm->id);
                $ratingdeloptions->contextid = $datacontext->id;
                ...
                // Delete any files that may exist.
                if ($contents = $DB->get_records('datac_content', array('recordid' => $record->id), '', 'id')) {
                    foreach ($contents as $content) {
                        $fs->delete_area_files($datacontext->id, 'mod_datac', 'content', $content->id);
                    }
                }
                $notenrolled[$record->userid] = true;

                core_tag_tag::remove_all_item_tags('mod_datac', 'datac_records', $record->id);

                $DB->delete_records('comments', array('itemid' => $record->id, 'commentarea' => 'database_entry'));
                $DB->delete_records('datac_content', array('recordid' => $record->id));
                $DB->delete_records('datac_records', array('id' => $record->id));
            }
        }
        $rs->close();
        $status[] = array('component'=>$componentstr, 'item'=>get_string('deletenotenrolled', 'datac'), 'error'=>false);
    }

    // remove all ratings
    if (!empty($data->reset_datac_ratings)) {
        if ($datas = $DB->get_records_sql($alldatassql, array($data->courseid))) {
            foreach ($datas as $dataid=>$unused) {
                if (!$cm = get_coursemodule_from_instance('datac', $dataid)) {
                    continue;
    ...
    // remove all comments
    if (!empty($data->reset_datac_comments)) {
        $DB->delete_records_select('comments', "itemid IN ($allrecordssql) AND commentarea='database_entry'", array($data->courseid));
        $status[] = array('component'=>$componentstr, 'item'=>get_string('deleteallcomments'), 'error'=>false);
    }

    // Remove all the tags.
    if (!empty($data->reset_datac_tags)) {
        if ($datas = $DB->get_records_sql($alldatassql, array($data->courseid))) {
            foreach ($datas as $dataid => $unused) {
                if (!$cm = get_coursemodule_from_instance('datac', $dataid)) {
                    continue;
                ...
                $context = context_module::instance($cm->id);
                core_tag_tag::delete_instances('mod_datac', null, $context->id);

            }
        }
        $status[] = array('component' => $componentstr, 'item' => get_string('tagsdeleted', 'datac'), 'error' => false);
    }
    ...
    if ($data->timeshift) {
        ...
        // See MDL-9367.
        shift_course_mod_dates('datac', array('timeavailablefrom', 'timeavailableto',
 ...
 * @return array
 */
function datac_get_extra_capabilities() {
 ...
 */
function datac_supports($feature) {
    switch($feature) {
 ...
 * @return string|void
 */
function datac_export_csv($export, $delimiter_name, $database, $count, $return=false) {
    global $CFG;
 ...
 * @return string
 */
function datac_export_xls($export, $dataname, $count) {
    global $CFG;
 ...
 * @param string
 */
function datac_export_ods($export, $dataname, $count) {
    global $CFG;
 ...
 * @return array
 */
function datac_get_exportdata($dataid, $fields, $selectedfields, $currentgroup=0, $context=null,
    ...
    $userdetails = $userdetails && has_capability('mod/datac:exportuserinfo', $context);
    ...
    if ($tags) {
        $exportdata[0][] = get_string('tags', 'datac');
    }
    if ($userdetails) {
    ...
    if ($time) {
        $exportdata[0][] = get_string('timeadded', 'datac');
        $exportdata[0][] = get_string('timemodified', 'datac');
    }
    if ($approval) {
        $exportdata[0][] = get_string('approved', 'datac');
    }

    $datarecords = $DB->get_records('datac_records', array('dataid'=>$dataid));
    ksort($datarecords);
    $line = 1;
    foreach($datarecords as $record) {
        // get content indexed by fieldid
        if ($currentgroup) {
            $select = 'SELECT c.fieldid, c.content, c.content1, c.content2, c.content3, c.content4 FROM {datac_content} c, {datac_records} r WHERE c.recordid = ? AND r.id = c.recordid AND r.groupid = ?';
            $where = array($record->id, $currentgroup);
        } else {
            $select = 'SELECT fieldid, content, content1, content2, content3, content4 FROM {datac_content} WHERE recordid = ?';
        ...
        if( $content = $DB->get_records_sql($select, $where) ) {
            ...
            if ($tags) {
                $itemtags = \core_tag_tag::get_item_tags_array('mod_datac', 'datac_records', $record->id);
 ...
 * @return array
 */
function datac_get_file_areas($course, $cm, $context) {
    return array('content' => get_string('areacontent', 'mod_datac'));
}
 ...
 */
function datac_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    global $CFG, $DB, $USER;
    ...
    if (is_null($itemid)) {
        require_once($CFG->dirroot.'/mod/datac/locallib.php');
        return new datac_file_info_container($browser, $course, $cm, $context, $areas, $filearea);
    }

    if (!$content = $DB->get_record('datac_content', array('id'=>$itemid))) {
        return null;
    }

    if (!$field = $DB->get_record('datac_fields', array('id'=>$content->fieldid))) {
        return null;
    }

    if (!$record = $DB->get_record('datac_records', array('id'=>$content->recordid))) {
        return null;
    }

    if (!$data = $DB->get_record('datac', array('id'=>$field->dataid))) {
        return null;
    }

    //check if approved
    if ($data->approval and !$record->approved and !datac_isowner($record) and !has_capability('mod/datac:approve', $context)) {
        return null;
    }
    ...
    $fieldobj = datac_get_field($field, $data, $cm);
    ...
    $fs = get_file_storage();
    if (!($storedfile = $fs->get_file($context->id, 'mod_datac', $filearea, $itemid, $filepath, $filename))) {
        return null;
    }
 ...
 */
function datac_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    global $CFG, $DB;
    ...
    if ($filearea === 'content') {
        ...
        if (!$content = $DB->get_record('datac_content', array('id'=>$contentid))) {
            return false;
        }
        ...
        $fullpath = "/$context->id/mod_datac/content/$content->id/$relativepath";
...
function datac_extend_navigation($navigation, $course, $module, $cm) {
    global $CFG, $OUTPUT, $USER, $DB;
    require_once($CFG->dirroot . '/mod/datac/locallib.php');

    $rid = optional_param('rid', 0, PARAM_INT);

    $data = $DB->get_record('datac', array('id'=>$cm->instance));
    $currentgroup = groups_get_activity_group($cm);
    $groupmode = groups_get_activity_groupmode($cm);

    $numentries = datac_numentries($data);
    $canmanageentries = has_capability('mod/datac:manageentries', context_module::instance($cm->id));

    if ($data->entriesleft = datac_get_entries_left_to_add($data, $numentries, $canmanageentries)) {
        $entriesnode = $navigation->add(get_string('entrieslefttoadd', 'datac', $data));
        $entriesnode->add_class('note');
    }

    $navigation->add(get_string('list', 'datac'), new moodle_url('/mod/datac/view.php', array('d'=>$cm->instance)));
    if (!empty($rid)) {
        $navigation->add(get_string('single', 'datac'), new moodle_url('/mod/datac/view.php', array('d'=>$cm->instance, 'rid'=>$rid)));
    } else {
        $navigation->add(get_string('single', 'datac'), new moodle_url('/mod/datac/view.php', array('d'=>$cm->instance, 'mode'=>'single')));
    }
    $navigation->add(get_string('search', 'datac'), new moodle_url('/mod/datac/view.php', array('d'=>$cm->instance, 'mode'=>'asearch')));
}
 ...
 */
function datac_extend_settings_navigation(settings_navigation $settings, navigation_node $datanode) {
    global $PAGE, $DB, $CFG, $USER;

    $data = $DB->get_record('datac', array("id" => $PAGE->cm->instance));

    $currentgroup = groups_get_activity_group($PAGE->cm);
    $groupmode = groups_get_activity_groupmode($PAGE->cm);

    if (datac_user_can_add_entry($data, $currentgroup, $groupmode, $PAGE->cm->context)) { // took out participation list here!
        if (empty($editentry)) { //TODO: undefined
            $addstring = get_string('add', 'datac');
        } else {
            $addstring = get_string('editentry', 'datac');
        }
        $datanode->add($addstring, new moodle_url('/mod/datac/edit.php', array('d'=>$PAGE->cm->instance)));
    }

    if (has_capability(DATA_CAP_EXPORT, $PAGE->cm->context)) {
        // The capability required to Export database records is centrally defined in 'lib.php'
        // and should be weaker than those required to edit Templates, Fields and Presets.
        $datanode->add(get_string('exportentries', 'datac'), new moodle_url('/mod/datac/export.php', array('d'=>$data->id)));
    }
    if (has_capability('mod/datac:manageentries', $PAGE->cm->context)) {
        $datanode->add(get_string('importentries', 'datac'), new moodle_url('/mod/datac/import.php', array('d'=>$data->id)));
    }

    if (has_capability('mod/datac:managetemplates', $PAGE->cm->context)) {
        $currenttab = '';
        if ($currenttab == 'list') {
        ...
        $templates = $datanode->add(get_string('templates', 'datac'));

        $templatelist = array ('listtemplate', 'singletemplate', 'asearchtemplate', 'addtemplate', 'rsstemplate', 'csstemplate', 'jstemplate');
        foreach ($templatelist as $template) {
            $templates->add(get_string($template, 'datac'), new moodle_url('/mod/datac/templates.php', array('d'=>$data->id,'mode'=>$template)));
        }

        $datanode->add(get_string('fields', 'datac'), new moodle_url('/mod/datac/field.php', array('d'=>$data->id)));
        $datanode->add(get_string('presets', 'datac'), new moodle_url('/mod/datac/preset.php', array('d'=>$data->id)));
    }
    ...
    if (!empty($CFG->enablerssfeeds) && !empty($CFG->data_enablerssfeeds) && $data->rssarticles > 0) {
        ...
        $url = new moodle_url(rss_get_url($PAGE->cm->context->id, $USER->id, 'mod_datac', $data->id));
 ...
 * @return bool
 */
function datac_presets_save($course, $cm, $data, $path) {
    global $USER;
    $fs = get_file_storage();
    ...
    $fs->create_file_from_string($filerecord, datac_presets_generate_xml($course, $cm, $data));
 ...
 */
function datac_presets_generate_xml($course, $cm, $data) {
    global $DB;
    ...
    if ($data->defaultsort > 0 && $sortfield = datac_get_field_from_id($data->defaultsort, $data)) {
    ...
    $fields = $DB->get_records('datac_fields', array('dataid'=>$data->id));
    ksort($fields);
    if (!empty($fields)) {
        foreach ($fields as $field) {
            $presetxmldata .= "<field>\n";
...
function datac_presets_export($course, $cm, $data, $tostorage=false) {
    global $CFG, $DB;

    $presetname = clean_filename($data->name) . '-preset-' . gmdate("Ymd_Hi");
    $exportsubdir = "mod_datac/presetexport/$presetname";
    make_temp_directory($exportsubdir);
    $exportdir = "$CFG->tempdir/$exportsubdir";

    // Assemble "preset.xml":
    $presetxmldata = datac_presets_generate_xml($course, $cm, $data);
    ...
    if (! is_directory_a_preset_c($exportdir)) {
        print_error('generateerror', 'datac');
    }
 ...
 * @return array
 */
function datac_comment_permissions($comment_param) {
    global $CFG, $DB;
    if (!$record = $DB->get_record('datac_records', array('id'=>$comment_param->itemid))) {
        throw new comment_exception('invalidcommentitemid');
    }
    if (!$data = $DB->get_record('datac', array('id'=>$record->dataid))) {
        throw new comment_exception('invalidid', 'datac');
    }
    if ($data->comments) {
 ...
 * @return boolean
 */
function datac_comment_validate($comment_param) {
    global $DB;
    ...
    if (!$cm = get_coursemodule_from_instance('datac', $data->id, $course->id)) {
        throw new comment_exception('invalidcoursemodule');
    }
    if (!$data->comments) {
        throw new comment_exception('commentsoff', 'datac');
    }
    $context = context_module::instance($cm->id);

    //check if approved
    if ($data->approval and !$record->approved and !datac_isowner($record) and !has_capability('mod/datac:approve', $context)) {
        throw new comment_exception('notapproved', 'datac');
    }
 ...
 */
function datac_page_type_list($pagetype, $parentcontext, $currentcontext) {
    $module_pagetype = array('mod-datac-*'=>get_string('page-mod-datac-x', 'datac'));
 ...
 */
function datac_get_all_recordids($dataid, $selectdata = '', $params = null) {
    global $DB;
    $initsql = 'SELECT r.id
                  FROM {datac_records} r
 ...
 */
function datac_get_advance_search_ids($recordids, $searcharray, $dataid) {
    ...
    foreach ($searchcriteria as $key) {
        $recordids = datac_get_recordids($key, $searcharray, $dataid, $recordids);
 ...
 */
function datac_get_recordids($alias, $searcharray, $dataid, $recordids) {
    global $DB;
    $searchcriteria = $alias;   // Keep the criteria.
    $nestsearch = $searcharray[$alias];
    // searching for content outside of mdl_datac_content
    if ($alias < 0) {
        $alias = '';
    }
    list($insql, $params) = $DB->get_in_or_equal($recordids, SQL_PARAMS_NAMED);
    $nestselect = 'SELECT c' . $alias . '.recordid
                     FROM {datac_content} c' . $alias . '
               INNER JOIN {datac_fields} f
                       ON f.id = c' . $alias . '.fieldid
               INNER JOIN {datac_records} r
    ...
    if (count($nestsearch->params) != 0) {
        ...
        foreach ($nestsearch->rawtagnames as $tagrawname) {
            $tagselect .= " INNER JOIN {tag_instance} ti_$i
                                    ON ti_$i.component = 'mod_datac'
                                   AND ti_$i.itemtype = 'datac_records'
 ...
 */
function datac_get_advanced_search_sql($sort, $data, $recordids, $selectdata, $sortorder) {
    global $DB;
    ...
    if ($sort == 0) {
        ...
        if ($sort <= 0) {
            ...
            $sortfield = datac_get_field_from_id($sort, $data);
    ...
    if ($sort > 0 or datac_get_field_from_id($sort, $data)) {
 ...
 */
function datac_user_can_delete_preset($context, $preset) {
    global $USER;

    if (has_capability('mod/datac:manageuserpresets', $context)) {
        return true;
    } else {
        $candelete = false;
 ...
 */
function datac_delete_record($recordid, $data, $courseid, $cmid) {
    global $DB, $CFG;

    if ($deleterecord = $DB->get_record('datac_records', array('id' => $recordid))) {
        if ($deleterecord->dataid == $data->id) {
            if ($contents = $DB->get_records('datac_content', array('recordid' => $deleterecord->id))) {
                foreach ($contents as $content) {
                    if ($field = datac_get_field_from_id($content->fieldid, $data)) {
                        $field->delete_content($content->recordid);
                    }
                }
                $DB->delete_records('datac_content', array('recordid'=>$deleterecord->id));
                $DB->delete_records('datac_records', array('id'=>$deleterecord->id));

                // Delete cached RSS feeds.
                if (!empty($CFG->enablerssfeeds)) {
                    require_once($CFG->dirroot.'/mod/datac/rsslib.php');
                    datac_rss_delete_file($data);
                }

                core_tag_tag::remove_all_item_tags('mod_datac', 'datac_records', $recordid);

                // Trigger an event for deleting this record.
                $event = \mod_datac\event\record_deleted::create(array(
                ...
                $event->add_record_snapshot('datac_records', $deleterecord);
                $event->trigger();
 ...
 * * datac_field_base[] fields The field objects to be update.
 */
function datac_process_submission(stdClass $mod, $fields, stdClass $datarecord) {
    $result = new stdClass();
    ...
    foreach ($fields as $fieldrecord) {
        ...
        $field = datac_get_field($fieldrecord, $mod);
        ...
        if ($field->field->required && !$fieldhascontent) {
            ...
            $result->fieldnotifications[$field->field->name][] = get_string('errormustsupplyvalue', 'datac');
    ...
    if ($emptyform) {
        // The form is empty.
        $result->generalnotifications[] = get_string('emptyaddform', 'datac');
    }
 ...
 * @return bool
 */
function datac_refresh_events($courseid = 0, $instance = null, $cm = null) {
    global $DB, $CFG;
    require_once($CFG->dirroot.'/mod/datac/locallib.php');
    ...
    if (isset($instance)) {
        if (!is_object($instance)) {
            $instance = $DB->get_record('datac', array('id' => $instance), '*', MUST_EXIST);
        }
        datac_set_events($instance);
        return true;
    }

    if ($courseid) {
        if (! $data = $DB->get_records("datac", array("course" => $courseid))) {
            return true;
        }
    } else {
        if (! $data = $DB->get_records("datac")) {
            return true;
        }
    }

    foreach ($data as $datum) {
        datac_set_events($datum);
    }
    return true;
}
 ...
 */
function datac_get_config($database, $key = null, $default = null) {
 ...
 */
function datac_set_config(&$database, $key, $value) {
    ...
    $config = datac_get_config($database);

    if (!isset($config->$key) || $config->$key !== $value) {
        $config->$key = $value;
        $database->config = json_encode($config);
        $DB->set_field('datac', 'config', $database->config, ['id' => $database->id]);
    }
}
/**
 ...
 */
function datac_update_completion_state($data, $course, $cm) {
 ...
 */
function datac_get_completion_state($course, $cm, $userid, $type) {
    global $DB, $PAGE;
    ...
    if ($data->completionentries) {
        $numentries = datac_numentries($data, $userid);
        // Check the number of entries required against the number of entries already made.
        if ($numentries >= $data->completionentries) {
            $result = true;
        } else {
 ...
 * @since Moodle 3.3
 */
function datac_view($data, $course, $cm, $context) {
    global $CFG;
    ...
    $event = \mod_datac\event\course_module_viewed::create($params);
    $event->add_record_snapshot('course_modules', $cm);
    $event->add_record_snapshot('course', $course);
    $event->add_record_snapshot('datac', $data);
    $event->trigger();
 ...
 */
function mod_datac_get_fontawesome_icon_map() {
    return [
        'mod_datac:field/checkbox' => 'fa-check-square-o',
        'mod_datac:field/date' => 'fa-calendar-o',
        'mod_datac:field/file' => 'fa-file',
        'mod_datac:field/latlong' => 'fa-globe',
        'mod_datac:field/menu' => 'fa-bars',
        'mod_datac:field/multimenu' => 'fa-bars',
        'mod_datac:field/number' => 'fa-hashtag',
        'mod_datac:field/picture' => 'fa-picture-o',
        'mod_datac:field/radiobutton' => 'fa-circle-o',
        'mod_datac:field/textarea' => 'fa-font',
        'mod_datac:field/text' => 'fa-i-cursor',
        'mod_datac:field/url' => 'fa-link',
    ];
}
 ...
 * @since Moodle 3.2
 */
function datac_check_updates_since(cm_info $cm, $from, $filter = array()) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/mod/datac/locallib.php');
    ...
    if (has_capability('mod/datac:manageentries', $cm->context) && groups_get_activity_groupmode($cm) != SEPARATEGROUPS) {
        $currentgroup = 0;
    }
    list($entries, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
        datac_search_entries($data, $cm, $cm->context, 'list', $currentgroup, '', null, null, 0, 0, true, $searcharray);
 ...
 */
function mod_datac_core_calendar_provide_event_action(calendar_event $event,
    ...
    $cm = get_fast_modinfo($event->courseid, $userid)->instances['datac'][$event->instance];
    ...
    return $factory->create_instance(
        get_string('add', 'datac'),
        new \moodle_url('/mod/datac/view.php', array('id' => $cm->id)),
        1,
        $actionable
    );
}
 ...
 */
function datac_get_coursemodule_info($coursemodule) {
    global $DB;

    $dbparams = ['id' => $coursemodule->instance];
    $fields = 'id, name, intro, introformat, completionentries, timeavailablefrom, timeavailableto';
    if (!$data = $DB->get_record('datac', $dbparams, $fields)) {
        return false;
    }
    ...
    if ($coursemodule->showdescription) {
        ...
        $result->content = format_module_intro('datac', $data, $coursemodule->id, false);
    }
 ...
 */
function mod_datac_get_completion_active_rule_descriptions($cm) {
    ...
    foreach ($cm->customdata['customcompletionrules'] as $key => $val) {
        switch ($key) {
            case 'completionentries':
                ...
                $descriptions[] = get_string('completionentriesdesc', 'datac', $val);
                break;
            default:
                break;
        }
 ...
 * @return array
 */
function mod_datac_core_calendar_get_valid_event_timestart_range(\calendar_event $event, \stdClass $instance) {
    $mindate = null;
    $maxdate = null;
    ...
    if ($event->eventtype == DATA_EVENT_TYPE_OPEN) {
        ...
        if (!empty($instance->timeavailableto)) {
            $maxdate = [
                ...
                get_string('openafterclose', 'datac')
            ];
        }
        ...
        if (!empty($instance->timeavailablefrom)) {
            $mindate = [
                ...
                get_string('closebeforeopen', 'datac')
            ];
        }
    }
 ...
 */
function mod_datac_core_calendar_event_timestart_updated(\calendar_event $event, \stdClass $data) {
    global $DB;

    if (empty($event->instance) || $event->modulename != 'datac') {
        return;
    }
