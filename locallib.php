 ...
 */
class datac_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function load_data() {
        global $DB, $USER;
        if (!$this->cm = get_coursemodule_from_id('datac', $this->id)) {
            throw new portfolio_caller_exception('invalidid', 'datac');
        }
        if (!$this->data = $DB->get_record('datac', array('id' => $this->cm->instance))) {
            throw new portfolio_caller_exception('invalidid', 'datac');
        }
        $fieldrecords = $DB->get_records('datac_fields', array('dataid' => $this->cm->instance), 'id');
        // populate objets for this databases fields
        $this->fields = array();
        foreach ($fieldrecords as $fieldrecord) {
            $tmp = datac_get_field($fieldrecord, $this->data);
            $this->fields[] = $tmp;
        ...
        $this->records = array();
        if ($this->recordid) {
            $tmp = $DB->get_record('datac_records', array('id' => $this->recordid));
            $tmp->content = $DB->get_records('datac_content', array('recordid' => $this->recordid));
            $this->records[] = $tmp;
        } else {
            $where = array('dataid' => $this->data->id);
            if (!has_capability('mod/datac:exportallentries', context_module::instance($this->cm->id))) {
                $where['userid'] = $USER->id; // get them all in case, we'll unset ones that aren't ours later if necessary
            }
            $tmp = $DB->get_records('datac_records', $where);
            foreach ($tmp as $t) {
                $t->content = $DB->get_records('datac_content', array('recordid' => $t->id));
                $this->records[] = $t;
            }
            $this->minecount = $DB->count_records('datac_records', array('dataid' => $this->data->id, 'userid' => $USER->id));
        }
        ...
        if ($leapwriter) {
            if (count($this->records) > 1) { // make a selection element to tie them all together
                $selection = new portfolio_format_leap2a_entry('datadb' . $this->data->id,
                    get_string('entries', 'datac') . ': ' . $this->data->name, 'selection');
    ...
    public function check_permissions() {
        if ($this->recordid) {
            if (datac_isowner($this->recordid)) {
                return has_capability('mod/datac:exportownentry', context_module::instance($this->cm->id));
            }
            return has_capability('mod/datac:exportentry', context_module::instance($this->cm->id));
        }
        if ($this->has_export_config() && !$this->get_export_config('mineonly')) {
            return has_capability('mod/datac:exportallentries', context_module::instance($this->cm->id));
        }
        return has_capability('mod/datac:exportownentry', context_module::instance($this->cm->id));
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'datac');
    }
    ...
    public function __wakeup() {
        ...
        foreach ($this->fieldtypes as $key => $field) {
            require_once($CFG->dirroot . '/mod/datac/field/' . $field .'/field.class.php');
    ...
    public static function has_files($data) {
        global $DB;
        $fieldrecords = $DB->get_records('datac_fields', array('dataid' => $data->id), 'id');
        // populate objets for this databases fields
        foreach ($fieldrecords as $fieldrecord) {
            $field = datac_get_field($fieldrecord, $data);
            if (is_callable(array($field, 'get_file'))) {
                return true;
            }
        }
    ...
    public function has_export_config() {
        ...
        return (empty($this->recordid) // multi-entry export
            ...
            && has_capability('mod/datac:exportallentries', context_module::instance($this->cm->id))); // they actually have a choice in the matter
    }
    ...
    public function export_config_form(&$mform, $instance) {
        ...
        $mform->addElement('selectyesno', 'mineonly', get_string('exportownentries', 'datac', (object)array('mine' => $this->minecount, 'all' => count($this->records))));
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class datac_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if ($child = $this->browser->get_file_info($this->context, 'mod_datac', $this->filearea, $record->itemid)) {
                $children[] = $child;
            }
 ...
 * @return void
 */
function datac_set_events($data) {
    global $DB, $CFG;
    ...
    if (!isset($data->coursemodule)) {
        $cm = get_coursemodule_from_instance('datac', $data->id, $data->course);
    ...
    if ($event->id = $DB->get_field('event', 'id',
            array('modulename' => 'datac', 'instance' => $data->id, 'eventtype' => $event->eventtype))) {
        if ($data->timeavailablefrom > 0) {
            // Calendar event exists so update it.
            $event->name         = get_string('calendarstart', 'datac', $data->name);
            $event->description  = format_module_intro('datac', $data, $data->coursemodule);
            $event->timestart    = $data->timeavailablefrom;
            $event->timesort     = $data->timeavailablefrom;
            $event->visible      = instance_is_visible('datac', $data);
    ...
    if ($event->id = $DB->get_field('event', 'id',
        ...
        if ($data->timeavailableto > 0) {
            // Calendar event exists so update it.
            $event->name         = get_string('calendarend', 'datac', $data->name);
            $event->description  = format_module_intro('datac', $data, $data->coursemodule);
 ...
 * @since  Moodle 3.3
 */
function datac_get_time_availability_status($data, $canmanageentries = null, $context = null) {
    $open = true;
    $closed = false;
    $warnings = array();
 ...
 * @since  Moodle 3.3
 */
function datac_require_time_available($data, $canmanageentries = null, $context = null) {

    list($available, $warnings) = datac_get_time_availability_status($data, $canmanageentries, $context);

    if (!$available) {
        $reason = current(array_keys($warnings));
        throw new moodle_exception($reason, 'datac', '', $warnings[$reason]);
    }
}
 ...
 * @since  Moodle 3.3
 */
function datac_get_entries_left_to_add($data, $numentries, $canmanageentries) {
 ...
 * @since  Moodle 3.3
 */
function datac_get_entries_left_to_view($data, $numentries, $canmanageentries) {
 ...
 *
 * This is a callback used by the tag area mod_datac/datac_records to search for data records
 ...
 */
function mod_datac_get_tagged_records($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = true, $page = 0) {
    ...
    $builder = new core_tag_index_builder('mod_datac', 'datac_records', $query, $params, $page * $perpage, $perpage + 1);
    $now = time();
    $entrycount = [];
    $activitygroupmode = [];
    $usergroups = [];
    ...
    while ($item = $builder->has_item_that_needs_access_check()) {
        ...
        if (!has_capability('mod/datac:viewentry', $context)) {
            $builder->set_accessible($item, false);
            continue;
        }
        ...
        if ($item->requiredentriestoview) {
            if (!isset($entrycount[$item->dataid])) {
                $entrycount[$item->dataid] = $DB->count_records('datac_records', array('dataid' => $item->dataid));
            }
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            ...
            $modinfo = get_fast_modinfo($item->courseid);
            $cm = $modinfo->get_cm($item->cmid);
            $pageurl = new moodle_url('/mod/datac/view.php', array(
                    'rid' => $item->id,
                    'd' => $item->dataid
            ));

            if (!isset($titlefields[$item->dataid])) {
                $titlefields[$item->dataid] = datac_get_tag_title_field($item->dataid);
            }

            $pagename = datac_get_tag_title_for_entry($titlefields[$item->dataid], $item);
        ...
        return new core_tag\output\tagindex($tag, 'mod_datac', 'datac_records', $content, $exclusivemode,
 ...
 * @return stdClass The field data from the 'datac_fields' table as well as it's priority
 */
function datac_get_tag_title_field($dataid) {
    global $DB, $CFG;

    $validfieldtypes = array('text', 'textarea', 'menu', 'radiobutton', 'checkbox', 'multimenu', 'url');
    $fields = $DB->get_records('datac_fields', ['dataid' => $dataid]);
    $template = $DB->get_field('datac', 'addtemplate', ['id' => $dataid]);
 ...
 *
 * @param stdClass $field The field from the 'datac_fields' table
 * @param stdClass $entry The entry from the 'datac_records' table
 * @return string The title of the entry
 */
function datac_get_tag_title_for_entry($field, $entry) {
    global $CFG, $DB;
    require_once($CFG->dirroot . '/mod/datac/field/' . $field->type . '/field.class.php');

    $classname = 'datac_field_' . $field->type;
    $sql = "SELECT dc.*
              FROM {datac_content} dc
        INNER JOIN {datac_fields} df
                ON dc.fieldid = df.id
             WHERE df.id = :fieldid
 ...
 * @since  Moodle 3.3
 */
function datac_search_entries($data, $cm, $context, $mode, $currentgroup, $search = '', $sort = null, $order = null, $page = 0,
    ...
    $approvecap = has_capability('mod/datac:approve', $context);
    $canmanageentries = has_capability('mod/datac:manageentries', $context);
    ...
    if (datac_get_entries_left_to_view($data, $numentries, $canmanageentries)) {
    ...
    if ($sort <= 0 or !$sortfield = datac_get_field_from_id($sort, $data)) {
        ...
        $tables = '{datac_content} c,{datac_records} r, {user} u ';
    ...
    $recordids = datac_get_all_recordids($data->id, $initialselect, $initialparams);
    $newrecordids = datac_get_advance_search_ids($recordids, $searcharray, $data->id);
    $selectdata = $where . $groupselect . $approveselect;

    if (!empty($advanced)) {
        $advancedsearchsql = datac_get_advanced_search_sql($sort, $data, $newrecordids, $selectdata, $sortorder);
    ...
    if (!$records = $DB->get_records_sql($sqlselect, $allparams, $page * $nowperpage, $nowperpage)) {
        ...
        if ($record) {         // Something was requested so try to show that at least (bug 5132)
            if (datac_can_view_record($data, $record, $currentgroup, $canmanageentries)) {
 ...
 * @since  Moodle 3.3
 */
function datac_can_view_record($data, $record, $currentgroup, $canmanageentries) {
    global $USER;
 ...
 * @since  Moodle 3.3
 */
function datac_get_field_instances($data) {
    global $DB;

    $instances = [];
    if ($fields = $DB->get_records('datac_fields', array('dataid' => $data->id), 'id')) {
        foreach ($fields as $field) {
            $instances[] = datac_get_field($field, $data);
        }
    }
    return $instances;
}
 ...
 * @since  Moodle 3.3
 */
function datac_build_search_array($data, $paging, $searcharray, $defaults = null, $fn = '', $ln = '') {
    global $DB;
    ...
    if (!empty($fields)) {
        foreach ($fields as $field) {
            $searchfield = datac_get_field_from_id($field->id, $data);
 ...
 * @since  Moodle 3.3
 */
function datac_approve_entry($entryid, $approve) {
    global $DB;

    $newrecord = new stdClass();
    $newrecord->id = $entryid;
    $newrecord->approved = $approve ? 1 : 0;
    $DB->update_record('datac_records', $newrecord);
}
 ...
 * @since  Moodle 3.3
 */
function datac_add_fields_contents_to_new_record($data, $context, $recordid, $fields, $datarecord, $processeddata) {
    global $DB;
    ...
    $DB->insert_records('datac_content', $records);
 ...
 * @since  Moodle 3.3
 */
function datac_update_record_fields_contents($data, $record, $context, $datarecord, $processeddata) {
    global $DB;
    ...
    $DB->update_record('datac_records', $record);
    ...
    // Trigger an event for updating this record.
    $event = \mod_datac\event\record_updated::create(array(
        'objectid' => $record->id,
        'context' => $context,
        'courseid' => $data->course,
        'other' => array(
...
function datac_datetimescan($str, $format){
    $utime = 0;
    if($date = strptime($str, $format)){
        $Y = $date['tm_year'] + 1900;
        $m = $date['tm_mon'] + 1;
        $d = $date['tm_mday'];
        $H = $date['tm_hour'];
        $M = $date['tm_min'];
        $s = $date['tm_sec'];
        $utime = strtotime("$Y/$m/$d $H:$M:$s");
    }
    return $utime;
}

function datac_import_log($datac, $obj, $line=null){
    global $CFG;

    if(is_writable($datac->importlog)){
        $filepath = $datac->importlog;
    }else{
        $filepath = $CFG->dataroot.'/datac_import.log';
    }
    if($line === null){
        $mes_head = "[".date("Y-m-d H:i:s")."]";
    }else{
        $mes_head = "[".date("Y-m-d H:i:s")."][".$line."]";
    }
    $mes_body = print_r($obj, true)."\n";
    error_log($mes_head . $mes_body, 3, $filepath);
}

function auto_import_exec($tergetdb){
    global $CFG, $DB;
    ...
    $course = $DB->get_record('course', array('id'=>$tergetdb->course), '*', MUST_EXIST);
    $cm     = get_coursemodule_from_instance('datac', $tergetdb->id, $course->id, false, MUST_EXIST);
    $course_context = context_course::instance($course->id);

    // ファイルを開く
    $csvfilepath = $CFG->dataroot."/".$tergetdb->csvfilepath;
    if(!is_readable($csvfilepath)){
        datac_import_log($tergetdb, "can not open terget file.[". $csvfilepath. "]", 1545);
        return false;
    }

    $filecontent = file_get_contents($csvfilepath);
    $readcount = $cir->load_csv_content($filecontent, "UTF-8", "comma");
    unset($filecontent);

    if (empty($readcount)) {
        $mes = "[$csvfilepath]".get_string('csvfailed', 'datac');
        datac_import_log($tergetdb, $mes);
        return false;
    } else {
        if (!$fieldnames = $cir->get_columns()) {
            $mes = "[$csvfilepath]".get_string('cannotreadtmpfile', 'error');
            datac_import_log($tergetdb, $mes);
        }
        $fieldnames = array_flip($fieldnames);
        // check the fieldnames are valid
        $rawfields = $DB->get_records('datac_fields', array('dataid' => $tergetdb->id), '', 'name, id, type');
        $fields = array();
        ...
        $safetoskipfields = array(get_string('user'), get_string('username'), get_string('email'),
            ...
            $tergetdb->useridfieldname, 
            $tergetdb->timecreatedfieldname, 
            $tergetdb->timemodifiedfieldname,
            $tergetdb->groupname
        );
        ...
        foreach ($fieldnames as $name => $id) {
            if (!isset($rawfields[$name])) {
                ...
                switch($name){
                    case $tergetdb->useridfieldname :
                        $optfields['userid'] = $id;
                        break;
                    case $tergetdb->timecreatedfieldname :
                        $optfields['timecreated'] = $id;
                        break;
                    case $tergetdb->timemodifiedfieldname :
                        $optfields['timemodified'] = $id;
                        break;
                    case $tergetdb->groupname :
                ...
                require_once("$CFG->dirroot/mod/datac/field/$field->type/field.class.php");
                $classname = 'datac_field_' . $field->type;
                $fields[$name] = new $classname($field, $tergetdb, $cm);
                $enablefields++;
            }
        }

        // ＤＢに存在しないフィールドがあった場合のメッセージ出力
        if (!empty($errorfield)) {
            datac_import_log($tergetdb, "[$csvfilepath] invalid field.($errorfield)");
        }

        // 有効なフィールドがなかった場合はメッセージを出して終了
        if($enablefields < 1){
            datac_import_log($tergetdb, "[$csvfilepath] Nothing enable fields");
            return false;
        }

        // ユーザ名、追加日時、更新日時のフィールドがなかった場合はエラーメッセージをログに出して終了
        if( !array_key_exists('userid', $optfields) ){
            datac_import_log($tergetdb, "[$csvfilepath] field not found (userid)");
            return false;
        }
        if( !array_key_exists('timecreated', $optfields) ){
            datac_import_log($tergetdb, "[$csvfilepath] field not found (timecreated)");
            return false;
        }
        if( !array_key_exists('timemodified', $optfields) ){
            datac_import_log($tergetdb, "[$csvfilepath] field not found (timemodified)");
            return false;
        }
        ...
        while ($record = $cir->next()) {
            ...
            if(empty($userrec)){
                // ユーザ情報が空の場合はそのレコードを登録せずログにメッセージを出力
                $mes = "[$csvfilepath:$recordcount]"." user not found"."(".$record[$optfields['userid']].")";
                datac_import_log($tergetdb, $mes);
                $recordserror++;
                continue;
            }else if( $tergetdb->limittoenrolled === "1" && !is_enrolled($course_context, $userrec->id) ){
                // コースに登録されていないユーザを登録しない＆ユーザがコースに登録されていない場合はそのレコードを登録せずログにメッセージを出力
                $mes = "[$csvfilepath:$recordcount]"." user not enrolled"."(".$record[$optfields['userid']].")";
                datac_import_log($tergetdb, $mes);
                $recordserror++;
                continue;
            }else{
            ...
            // エントリの追加日時と修正日時をＤＢに格納
            $cre = $record[$optfields['timecreated']];
            $mod = $record[$optfields['timemodified']];
            if($cre === NULL || $cre === ''){
            }else{
                $opt['timecreated'] = datac_datetimescan($cre, $tergetdb->timecreatedformat);
            }
            if($mod === NULL || $mod === ''){
            }else{
                $opt['timemodified'] = datac_datetimescan($mod, $tergetdb->timemodifiedformat);
            }

            // グループid関係の処理
            if( empty($tergetdb->groupname) 
            ...
            // レコードの登録
            if ($recordid = datac_add_record($tergetdb, $groupid, $opt)) {  // add instance to datac_record
                foreach ($fields as $field) {
                    $fieldid = $fieldnames[$field->field->name];
                    if (isset($record[$fieldid])) {
                        // 12/06 type が date であった場合、フォーマットによっては値を変換する
                        if($field->field->type == 'date'){
                            if($tergetdb->date_format_type === 'YYYY-MM-DD'){
