...
class mod_datac_renderer extends plugin_renderer_base {

    public function import_setting_mappings($datamodule, datac_preset_importer $importer) {

        $strblank = get_string('blank', 'datac');
        $strcontinue = get_string('continue');
        $strwarning = get_string('mappingwarning', 'datac');
        $strfieldmappings = get_string('fieldmappings', 'datac');
        ...
        if ($importer instanceof datac_preset_existing_importer) {
        ...
        if (!empty($newfields)) {
            $html .= $this->output->heading_with_help($strfieldmappings, 'fieldmappings', 'datac', '', '', 3);
            ...
            foreach ($newfields as $nid => $newfield) {
                ...
                foreach ($currentfields as $cid => $currentfield) {
                    ...
                    if ($currentfield->name == $newfield->name) {
                        $row[1] .= html_writer::tag('option', get_string('mapexistingfield', 'datac', $currentfield->name), array('value'=>$cid, 'selected'=>'selected'));
                        $selected=true;
                    } else {
                        $row[1] .= html_writer::tag('option', get_string('mapexistingfield', 'datac', $currentfield->name), array('value'=>$cid));
                    }
                }

                if ($selected) {
                    $row[1] .= html_writer::tag('option', get_string('mapnewfield', 'datac'), array('value'=>'-1'));
                } else {
                    $row[1] .= html_writer::tag('option', get_string('mapnewfield', 'datac'), array('value'=>'-1', 'selected'=>'selected'));
                }
            ...
            $html .= $this->output->notification(get_string('nodefinedfields', 'datac'));
        }

        $html .= html_writer::start_tag('div', array('class'=>'overwritesettings'));
        $html .= html_writer::tag('label', get_string('overwritesettings', 'datac'), array('for' => 'overwritesettings'));
