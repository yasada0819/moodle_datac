...
/**
 * Data generator class for mod_datac.
 *
 * @package    mod_datac
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_datac_generator extends testing_module_generator {
    ...
    /**
     * Creates a field for a mod_datac instance.
     *
     * @param StdClass $record
     * @param mod_datac $data
     * @return datac_field_{type}
     */
    public function create_field(stdClass $record = null, $data = null) {
        ...
        $field = datac_get_field($record, $data);
        $field->insert_field();

        datac_generate_default_template($data, 'addtemplate', 0, false, true);
     ...
     *
     * @param stdClass $data record from table {datac}
     * @param array $contents
     * @param int $groupid
     * @param array $tags
     * @param array $options
     * @return int id of the generated record in table {datac_records}
     */
    public function create_entry($data, array $contents, $groupid = 0, $tags = [], array $options = null) {
        ...
        $recordid = datac_add_record($data, $groupid);

        if (isset($options['approved'])) {
            datac_approve_entry($recordid, !empty($options['approved']));
        } else {
            $approved = null;
        }
        ...
        foreach ($fields as $field) {
            ...
            $field = datac_get_field($field, $data);
            ...
            if ($field->type === 'date') {
                ...
                if (strlen($itemid) && !is_numeric($itemid)) {
                    ...
                    get_file_storage()->create_file_from_string(['component' => 'user', 'filearea' => 'draft',
                        ...
                        'filename' => $filename],
                        file_get_contents($CFG->dirroot.'/mod/datac/pix/icon.png'));
                }
        ...
        foreach ($contents as $fieldid => $content) {
            $field = datac_get_field_from_id($fieldid, $data);
        ...
        if (!empty($tags)) {
            ...
            core_tag_tag::set_item_tags('mod_datac', 'datac_records', $recordid,
