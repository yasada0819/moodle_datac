 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_datac_generator_testcase extends advanced_testcase {
    public function test_generator() {
        global $DB;

        $this->resetAfterTest(true);

        $this->assertEquals(0, $DB->count_records('datac'));

        $course = $this->getDataGenerator()->create_course();

        /** @var mod_datac_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_datac');
        $this->assertInstanceOf('mod_datac_generator', $generator);
        $this->assertEquals('datac', $generator->get_modulename());

        $generator->create_instance(array('course' => $course->id));
        $generator->create_instance(array('course' => $course->id));
        $data = $generator->create_instance(array('course' => $course->id));
        $this->assertEquals(3, $DB->count_records('datac'));

        $cm = get_coursemodule_from_instance('datac', $data->id);
        $this->assertEquals($data->id, $cm->instance);
        $this->assertEquals('datac', $cm->modname);
        ...
        $gitem = $DB->get_record('grade_items', array('courseid' => $course->id, 'itemtype' => 'mod',
                'itemmodule' => 'datac', 'iteminstance' => $data->id));
    ...
    public function test_create_field() {
        ...
        $data = $generator->create_instance(array('course' => $course->id));
        $this->assertEquals(1, $DB->count_records('datac'));
        ...
        foreach ($fieldtypes as $fieldtype) {
            ...
            $this->assertInstanceOf('datac_field_' . $fieldtype, ${$fieldname});
            $count++;
        }

        $this->assertEquals(count($fieldtypes), $DB->count_records('datac_fields', array('dataid' => $data->id)));

        $addtemplate = $DB->get_record('datac', array('id' => $data->id), 'addtemplate');
    ...
    public function test_create_entry() {
        ...
        $this->setUser($user1);
        $datarecordid = $this->getDataGenerator()->get_plugin_generator('mod_datac')->create_entry($data,
            $fieldcontents, $groupa->id, $tags);

        $this->assertEquals(1, $DB->count_records('datac_records', array('dataid' => $data->id)));
        $this->assertEquals(count($contents), $DB->count_records('datac_content', array('recordid' => $datarecordid)));

        $entry = $DB->get_record('datac_records', array('id' => $datarecordid));
        $this->assertEquals($entry->groupid, $groupa->id);

        $contents = $DB->get_records('datac_content', array('recordid' => $datarecordid), 'id');
        ...
        $this->assertEquals(array('Cats', 'mice'),
            array_values(core_tag_tag::get_item_tags_array('mod_datac', 'datac_records', $datarecordid)));
    }
}
