 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_datac_lib_testcase extends advanced_testcase {
    ...
    public function test_data_completion() {
         ...
         */
        $module = $this->getDataGenerator()->create_module('datac', $record, array('completion' => 2, 'completionentries' => 1));

        $cm = get_coursemodule_from_instance('datac', $module->id, $course->id);
        ...
        $field = datac_get_field_new('text', $module);
        ...
        $field->insert_field();
        $recordid = datac_add_record($module);

        $datacontent = array();
        $datacontent['fieldid'] = $field->field->id;
        $datacontent['recordid'] = $recordid;
        $datacontent['content'] = 'Asterix';
        $contentid = $DB->insert_record('datac_content', $datacontent);
    ...
    public function test_datac_delete_record() {
        global $DB;
        ...
        $module = $this->getDataGenerator()->create_module('datac', $record);
        ...
        $data = $DB->get_records('datac', array('id' => $module->id));
        $this->assertEquals(1, count($data));

        $datacontent = $DB->get_records('datac_content', array('id' => $contentid));
        $this->assertEquals(1, count($datacontent));

        $datacfields = $DB->get_records('datac_fields', array('id' => $field->field->id));
        $this->assertEquals(1, count($datacfields));

        $datarecords = $DB->get_records('datac_records', array('id' => $recordid));
        $this->assertEquals(1, count($datarecords));

        // Test to see if a failed delete returns false.
        $result = datac_delete_record(8798, $module, $course->id, $cm->id);
        $this->assertFalse($result);

        // Delete the record.
        $result = datac_delete_record($recordid, $module, $course->id, $cm->id);
    ...
    public function test_data_comment_created_event() {
        ...
        $this->assertInstanceOf('\mod_datac\event\comment_created', $event);
        $this->assertEquals($context, $event->get_context());
        $url = new moodle_url('/mod/datac/view.php', array('id' => $cm->id));
    ...
    public function test_data_comment_deleted_event() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_datac\event\comment_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $url = new moodle_url('/mod/datac/view.php', array('id' => $module->cmid));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    }

    /**
     * Checks that datac_user_can_manage_entry will return true if the user
     * has the mod/datac:manageentries capability.
     */
    public function test_datac_user_can_manage_entry_return_true_with_capability() {
        ...
        assign_capability('mod/datac:manageentries', CAP_ALLOW, $roleid, $context);

        $this->assertTrue(datac_user_can_manage_entry($record, $data, $context),
            'datac_user_can_manage_entry() returns true if the user has mod/datac:manageentries capability');
    }

    /**
     * Checks that datac_user_can_manage_entry will return false if the data
     * is set to readonly.
     */
    public function test_datac_user_can_manage_entry_return_false_readonly() {
        ...
        // the other checks.
        assign_capability('mod/datac:manageentries', CAP_PROHIBIT, $roleid, $context);
        ...
        $this->assertFalse(datac_user_can_manage_entry($record, $data, $context),
            'datac_user_can_manage_entry() returns false if the data is read only');
    }

    /**
     * Checks that datac_user_can_manage_entry will return false if the record
     * can't be found in the database.
     */
    public function test_datac_user_can_manage_entry_return_false_no_record() {
        ...
        $this->assertFalse(datac_user_can_manage_entry(1, $data, $context),
            'datac_user_can_manage_entry() returns false if the record cannot be found');
    }
     ...
     */
    public function test_datac_user_can_manage_entry_return_false_not_owned_record() {
        ...
        $this->assertFalse(datac_user_can_manage_entry($record, $data, $context),
            'datac_user_can_manage_entry() returns false if the record isnt owned by the user');
    }

    /**
     * Checks that datac_user_can_manage_entry will return true if the data
     * doesn't require approval.
     */
    public function test_datac_user_can_manage_entry_return_true_data_no_approval() {
        ...
        $this->assertTrue(datac_user_can_manage_entry($record, $data, $context),
            'datac_user_can_manage_entry() returns true if the record doesnt require approval');
    }

    /**
     * Checks that datac_user_can_manage_entry will return true if the record
     * isn't yet approved.
     */
    public function test_datac_user_can_manage_entry_return_true_record_unapproved() {
        ...
        $this->assertTrue(datac_user_can_manage_entry($record, $data, $context),
            'datac_user_can_manage_entry() returns true if the record is not yet approved');
    }

    /**
     * Checks that datac_user_can_manage_entry will return the 'manageapproved'
     * value if the record has already been approved.
     */
    public function test_datac_user_can_manage_entry_return_manageapproved() {
        ...
        $canmanageentry = datac_user_can_manage_entry($record, $data, $context);
        ...
        $this->assertEquals($data->manageapproved, $canmanageentry,
            'datac_user_can_manage_entry() returns the manageapproved setting on approved records');
    }
    ...
    /**
     * Tests for mod_datac_rating_can_see_item_ratings().
     *
     * @throws coding_exception
     * @throws rating_exception
     */
    public function test_mod_datac_rating_can_see_item_ratings() {
        global $DB;
        ...
        $data = $this->getDataGenerator()->create_module('datac', array('course' => $course->id));
        ...
        // Add data.
        $field = datac_get_field_new('text', $data);
        ...
        $recordid1 = datac_add_record($data, 0);

        $datacontent = array();
        $datacontent['fieldid'] = $field->field->id;
        $datacontent['recordid'] = $recordid1;
        $datacontent['content'] = 'Obelix';
        $DB->insert_record('datac_content', $datacontent);

        $recordid = datac_add_record($data, $group1->id);
        ...
        $this->setUser($user1);
        $this->assertTrue(mod_datac_rating_can_see_item_ratings($params));
        $this->assertTrue(mod_datac_rating_can_see_item_ratings($params1));
        $this->setUser($user2);
        $this->assertTrue(mod_datac_rating_can_see_item_ratings($params));
        $this->assertTrue(mod_datac_rating_can_see_item_ratings($params1));
        $this->setUser($user3);
        $this->assertFalse(mod_datac_rating_can_see_item_ratings($params));
        $this->assertTrue(mod_datac_rating_can_see_item_ratings($params1));
        $this->setUser($user4);
        $this->assertFalse(mod_datac_rating_can_see_item_ratings($params));
        $this->assertTrue(mod_datac_rating_can_see_item_ratings($params1));
    ...
    /**
     * Tests for mod_datac_refresh_events.
     */
    public function test_datac_refresh_events() {
        global $DB;
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $course = $this->getDataGenerator()->create_course();
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_datac');
        ...
        $this->assertTrue(datac_refresh_events($course->id));
        $eventparams = array('modulename' => 'datac', 'instance' => $data->id, 'eventtype' => 'open');
        $openevent = $DB->get_record('event', $eventparams, '*', MUST_EXIST);
        $this->assertEquals($openevent->timestart, $timeopen);

        $eventparams = array('modulename' => 'datac', 'instance' => $data->id, 'eventtype' => 'close');
        ...
        $this->assertTrue(datac_refresh_events());
        $eventparams = array('modulename' => 'datac');
        $events = $DB->get_records('event', $eventparams);
        foreach ($events as $event) {
            if ($event->modulename === 'datac' && $event->instance === $data->id && $event->eventtype === 'open') {
                $this->assertEquals($event->timestart, $timeopen);
            }
            if ($event->modulename === 'datac' && $event->instance === $data->id && $event->eventtype === 'close') {
    ...
    /**
     * Data provider for tests of datac_get_config.
     *
     * @return array
     */
    public function datac_get_config_provider() {
        $initialdata = (object) [
            'template_foo' => true,
            'template_bar' => false,
            'template_baz' => null,
    ...
    /**
     * Tests for datac_get_config.
     *
     * @dataProvider    datac_get_config_provider
     * @param   array   $funcargs       The args to pass to datac_get_config
     * @param   mixed   $expectation    The expected value
     */
    public function test_datac_get_config($funcargs, $expectation) {
        $this->assertEquals($expectation, call_user_func_array('datac_get_config', $funcargs));
    }

    /**
     * Data provider for tests of datac_set_config.
     *
     * @return array
     */
    public function datac_set_config_provider() {
    ...
    /**
     * Tests for datac_set_config.
     *
     * @dataProvider    datac_set_config_provider
     ...
     */
    public function test_datac_set_config($database, $key, $value, $expectupdate, $newconfigvalue) {
        global $DB;
        ...
        // Perform the update.
        datac_set_config($database, $key, $value);
     ...
     * @return void
     */
    public function test_datac_view() {
        global $CFG;
        ...
        datac_view($data, $course, $cm, $context);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_datac\event\course_module_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $moodleurl = new \moodle_url('/mod/datac/view.php', array('id' => $cm->id));
    ...
    public function test_mod_datac_get_tagged_records() {
        $this->resetAfterTest();
        $this->setAdminUser();

        // Setup test data.
        $datagenerator = $this->getDataGenerator()->get_plugin_generator('mod_datac');
        ...
        $data1 = $this->getDataGenerator()->create_module('datac', array('course' => $course1->id, 'approval' => true));
        ...
        $res = mod_datac_get_tagged_records($tag, false, 0, 0, 1, 0);
    ...
    public function test_mod_datac_get_tagged_records_approval() {
        global $DB;
        ...
        $data1 = $this->getDataGenerator()->create_module('datac', array('course' => $course1->id));
        $field1 = $datagenerator->create_field($fieldrecord, $data1);
        $data2 = $this->getDataGenerator()->create_module('datac', array('course' => $course2->id, 'approval' => true));
        ...
        $DB->update_record('datac_records', $recordtoupdate);
    ...
    public function test_mod_datac_get_tagged_records_time() {
        global $DB;
        ...
        $data1 = $this->getDataGenerator()->create_module('datac', array('course' => $course1->id, 'approval' => true));
        $field1 = $datagenerator->create_field($fieldrecord, $data1);
        $data2 = $this->getDataGenerator()->create_module('datac', array('course' => $course2->id,
    ...
    public function test_mod_datac_get_tagged_records_course_enrolment() {
        global $DB;
    ...
    public function test_mod_datac_get_tagged_records_course_groups() {
        global $DB;
    ...
    public function test_check_updates_since() {
        ...
        $data = $this->getDataGenerator()->create_module('datac', $record);
        $cm = get_coursemodule_from_instance('datac', $data->id, $course->id);
        $cm = cm_info::create($cm);
        $this->setUser($student);

        // Check that upon creation, the updates are only about the new configuration created.
        $onehourago = time() - HOURSECS;
        $updates = datac_check_updates_since($cm, $onehourago);
        ...
        $fields = $DB->get_records('datac_fields', array('dataid' => $data->id), 'id');
        ...
        $records = $DB->get_records('datac_records', array('dataid' => $data->id));
    ...
    public function test_data_core_calendar_provide_event_action_in_hidden_section() {
        ...
        $actionevent = mod_datac_core_calendar_provide_event_action($event, $factory, $student->id);
    ...
    public function test_data_core_calendar_provide_event_action_for_non_user() {
        ...
        $actionevent = mod_datac_core_calendar_provide_event_action($event, $factory);
    ...
    public function test_data_core_calendar_provide_event_action_open() {
        ...
        $this->assertEquals(get_string('add', 'datac'), $actionevent->get_name());
     ...
     */
    public function test_mod_datac_completion_get_active_rule_descriptions() {
        $this->resetAfterTest();
        $this->setAdminUser();

        // Two activities, both with automatic completion. One has the 'completionentries' rule, one doesn't.
        $course = $this->getDataGenerator()->create_course(['enablecompletion' => 2]);
        $data1 = $this->getDataGenerator()->create_module('datac', [
            'course' => $course->id,
            'completion' => 2,
            'completionentries' => 3
        ]);
        $data2 = $this->getDataGenerator()->create_module('datac', [
            'course' => $course->id,
            'completion' => 2,
            'completionentries' => 0
        ]);
        $cm1 = cm_info::create(get_coursemodule_from_instance('datac', $data1->id));
        $cm2 = cm_info::create(get_coursemodule_from_instance('datac', $data2->id));
        ...
        $activeruledescriptions = [get_string('completionentriesdesc', 'datac', 3)];
        $this->assertEquals(mod_datac_get_completion_active_rule_descriptions($cm1), $activeruledescriptions);
        $this->assertEquals(mod_datac_get_completion_active_rule_descriptions($cm2), []);
        $this->assertEquals(mod_datac_get_completion_active_rule_descriptions($moddefaults), $activeruledescriptions);
        $this->assertEquals(mod_datac_get_completion_active_rule_descriptions(new stdClass()), []);
    }

    /**
     * An unknown event type should not change the data instance.
     */
    public function test_mod_datac_core_calendar_event_timestart_updated_unknown_event() {
        global $CFG, $DB;
        ...
        $datagenerator = $generator->get_plugin_generator('mod_datac');
        $timeopen = time();
        ...
        mod_datac_core_calendar_event_timestart_updated($event, $data);
        $data = $DB->get_record('datac', ['id' => $data->id]);
     ...
     */
    public function test_mod_datac_core_calendar_event_timestart_updated_open_event() {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/calendar/lib.php");
     ...
     */
    public function test_mod_datac_core_calendar_event_timestart_updated_close_event() {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/calendar/lib.php");
     ...
     */
    public function test_mod_datac_core_calendar_get_valid_event_timestart_range_unknown_event() {
        global $CFG;
        require_once($CFG->dirroot . "/calendar/lib.php");
        ...
        list ($min, $max) = mod_datac_core_calendar_get_valid_event_timestart_range($event, $data);
        $this->assertNull($min);
        $this->assertNull($max);
    }
     ...
     */
    public function test_mod_datac_core_calendar_get_valid_event_timestart_range_open_event() {
        global $CFG;
        require_once($CFG->dirroot . "/calendar/lib.php");
     ...
     */
    public function test_mod_datac_core_calendar_get_valid_event_timestart_range_close_event() {
        global $CFG;
    ...
    public function test_creation_with_no_calendar_capabilities() {
        ...
        $generator = self::getDataGenerator()->get_plugin_generator('mod_datac');
