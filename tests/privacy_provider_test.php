...
use mod_datac\privacy\provider;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_datac_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    ...
    protected function setUp() {
        ...
        $cm = get_coursemodule_from_instance('datac', $datamodule->id);
        ...
        $record1id = $this->generate_datac_record($datamodule);
        $this->generate_datac_record($datamodule);
    ...
    /**
     * Get mod_datac generator
     *
     * @return mod_datac_generator
     */
    protected function get_generator() {
        return $this->getDataGenerator()->get_plugin_generator('mod_datac');
    }
     ...
     * @return mixed
     */
    protected function generate_datac_record($datamodule) {
        global $DB;
        ...
        $count = 0;
        $fieldcontents = array();
        $fields = $DB->get_records('datac_fields', array('dataid' => $datamodule->id), 'id');
    ...
    public function test_get_metadata() {
        $collection = new collection('mod_datac');
        $newcollection = provider::get_metadata($collection);
        $itemcollection = $newcollection->get_collection();
        $this->assertCount(7, $itemcollection);

        $table = reset($itemcollection);
        $this->assertEquals('datac_records', $table->get_name());

        $table = next($itemcollection);
        $this->assertEquals('datac_content', $table->get_name());
    }
    ...
    public function test_get_contexts_for_userid() {
        $cm = get_coursemodule_from_instance('datac', $this->datamodule->id);
    ...
    public function test_export_for_context() {
        ...
        $cm = get_coursemodule_from_instance('datac', $this->datamodule->id);
        $cmcontext = context_module::instance($cm->id);
        $records = $DB->get_records_select('datac_records', 'userid = :userid ORDER BY id', ['userid' => $this->student->id]);
        $record = reset($records);
        $contents = $DB->get_records('datac_content', ['recordid' => $record->id]);

        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student->id, $cmcontext, 'mod_datac');
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $appctxt = new \core_privacy\local\request\approved_contextlist($this->student, 'mod_datac', [$cmcontext->id]);
    ...
    public function test_delete_data_for_users() {
        ...
        $appctxt = new \core_privacy\local\request\approved_contextlist($this->student2, 'mod_datac', [$cmcontext->id]);
        provider::export_user_data($appctxt);
        $this->assertTrue($this->get_writer($cmcontext)->has_any_data());

        // Delete data for student 1 and 2.
        $approvedlist = new approved_userlist($cmcontext, 'mod_datac', $userstodelete);
        ...
        $appctxt = new \core_privacy\local\request\approved_contextlist($this->student3, 'mod_datac', [$cmcontext->id]);
        provider::export_user_data($appctxt);
