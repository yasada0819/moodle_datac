...
/**
 * Unit tests for data_get_all_recordsids(), datac_get_advance_search_ids(), data_get_record_ids(),
 * and datac_get_advanced_search_sql()
 *
 * @package    mod_datac
 * @category   phpunit
...
require_once($CFG->dirroot . '/mod/datac/tests/generator/lib.php');

/**
 * Unit tests for {@see datac_get_all_recordids()}.
 *                {@see data_get_advanced_search_ids()}
 *                {@see data_get_record_ids()}
 *                {@see datac_get_advanced_search_sql()}
 *
 * @package    mod_datac
 * @copyright  2012 Adrian Greeve
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_datac_search_test extends advanced_testcase {
    /**
    ...
    protected function setUp() {
        ...
        $this->databaseentryareaid = \core_search\manager::generate_areaid('mod_datac', 'entry');
    ...
    /**
     * Test 1: The function datac_get_all_recordids.
     *
     * Test 2: This tests the datac_get_advance_search_ids() function. The function takes a set
     ...
     *
     * Test 3: This function tests datac_get_recordids(). This is the function that is nested in the last
     * function (see datac_get_advance_search_ids). This function takes a couple of
     ...
     *
     * Test 4: datac_get_advanced_search_sql provides an array which contains an sql string to be used for displaying records
     ...
     *
     * Test 5: Returning to datac_get_all_recordids(). Here we are ensuring that the total amount of record ids is reduced to
     ...
     *
     * Test 6: datac_get_all_recordids() again. This time we are testing approved database records. We only want to
    ...
    public function test_advanced_search_sql_section() {
        ...
        $files = array(
                'datac_fields'  => __DIR__.'/fixtures/test_datac_fields.csv',
                'datac_records' => __DIR__.'/fixtures/test_datac_records.csv',
                'datac_content' => __DIR__.'/fixtures/test_datac_content.csv',
        );
        $this->loadDataSet($this->createCsvDataSet($files));
        // Set dataid to the correct value now the data has been inserted by csv file.
        $DB->execute('UPDATE {datac_fields} SET dataid = ?', array($data->id));
        $DB->execute('UPDATE {datac_records} SET dataid = ?', array($data->id));
        ...
        // Test 1
        $recordids = datac_get_all_recordids($this->recorddata->id);
        $this->assertEquals(count($recordids), $this->datarecordcount);

        // Test 2
        $key = array_keys($this->recordsearcharray);
        $alias = $key[0];
        $newrecordids = datac_get_recordids($alias, $this->recordsearcharray, $this->recorddata->id, $recordids);
        $this->assertEquals($this->datarecordset, $newrecordids);

        // Test 3
        $newrecordids = datac_get_advance_search_ids($recordids, $this->recordsearcharray, $this->recorddata->id);
        $this->assertEquals($this->datarecordset, $newrecordids);

        // Test 3.1
        $resultrecordids = datac_get_advance_search_ids(array(), $this->recordsearcharray, $this->recorddata->id);
        $this->assertEmpty($resultrecordids);

        // Test 4
        $sortorder = 'ORDER BY r.timecreated ASC , r.id ASC';
        $html = datac_get_advanced_search_sql('0', $this->recorddata, $newrecordids, '', $sortorder);
        ...
        $recordids = datac_get_all_recordids($this->recorddata->id, $groupsql, $params);
        $this->assertEquals($this->groupdatarecordcount, count($recordids));

        // Test 6
        $approvesql = ' AND r.approved=1 ';
        $recordids = datac_get_all_recordids($this->recorddata->id, $approvesql, $params);
    ...
    public function test_advanced_search_tags() {
        ...
        $recordids = datac_get_all_recordids($data1->id);
        $newrecordids = datac_get_advance_search_ids($recordids, $searcharray, $data1->id);
    ...
    public function test_data_entries_indexing() {
        ...
        $this->assertInstanceOf('\mod_datac\search\entry', $searcharea);
        ...
        $data1 = $this->getDataGenerator()->create_module('datac', $record);
        ...
        $this->create_default_datac_fields($fieldtypes, $data1);

        $data1record1id = $this->create_default_datac_record($data1);
        // All records.
        ...
        $data2 = $this->getDataGenerator()->create_module('datac', ['course' => $course1->id]);
        $this->create_default_datac_fields($fieldtypes, $data2);
        $this->create_default_datac_record($data2);
    ...
    public function test_data_entries_document() {
        ...
        // Second Case.
        $data2 = $this->getDataGenerator()->create_module('datac', $record);
        ...
        $data2record1id = $this->create_default_datac_record($data2);
        ...
        // Third Case.
        $data3 = $this->getDataGenerator()->create_module('datac', $record);

        $fieldtypes = array( 'url' );

        $this->create_default_datac_fields($fieldtypes, $data3);

        $data3record1id = $this->create_default_datac_record($data3);
        ...
        // Fourth Case.
        $data4 = $this->getDataGenerator()->create_module('datac', $record);

        $fieldtypes = array( array('date', 1), array('text', 1));

        $this->create_default_datac_fields($fieldtypes, $data4);

        $data4record1id = $this->create_default_datac_record($data4);
        ...
        // Fifth Case.
        $data5 = $this->getDataGenerator()->create_module('datac', $record);
        ...
        $this->create_default_datac_fields($fieldtypes, $data5);

        $data5record1id = $this->create_default_datac_record($data5);
        ...
        // Sixth Case.
        $data6 = $this->getDataGenerator()->create_module('datac', $record);

        $fieldtypes = array( array('date', 1), array('number', 1));

        $this->create_default_datac_fields($fieldtypes, $data6);

        $data6record1id = $this->create_default_datac_record($data6);
        ...
        // Seventh Case.
        $data7 = $this->getDataGenerator()->create_module('datac', $record);

        $fieldtypes = array( array('date', 1), array('number', 1),
                array('text', 0), array('textarea', 0));

        $this->create_default_datac_fields($fieldtypes, $data7);

        $data7record1id = $this->create_default_datac_record($data7);
        ...
        // Eight Case.
        $data8 = $this->getDataGenerator()->create_module('datac', $record);

        $fieldtypes = array('url', 'url', 'url', 'text');

        $this->create_default_datac_fields($fieldtypes, $data8);

        $data8record1id = $this->create_default_datac_record($data8);
        ...
        // Ninth Case.
        $data9 = $this->getDataGenerator()->create_module('datac', $record);

        $fieldtypes = array('radiobutton', 'menu', 'multimenu');

        $this->create_default_datac_fields($fieldtypes, $data9);

        $data9record1id = $this->create_default_datac_record($data9);
        ...
        // Tenth Case.
        $data10 = $this->getDataGenerator()->create_module('datac', $record);

        $fieldtypes = array('checkbox', 'textarea', 'multimenu');

        $this->create_default_datac_fields($fieldtypes, $data10);

        $data10record1id = $this->create_default_datac_record($data10);
    ...
    public function test_data_entries_group_support() {
        ...
        $data = self::getDataGenerator()->create_module('datac', ['course' => $course->id,
                'groupmode' => SEPARATEGROUPS]);
        $fieldtypes = ['text', 'textarea'];
        $this->create_default_datac_fields($fieldtypes, $data);
    ...
    public function test_data_entries_access() {
        ...
        $cm = get_coursemodule_from_instance('datac', $data2->id, $course->id);
        $cm->groupmode = '1';
        ...
        $data2record1id = $this->create_default_datac_record($data2, $groupa->id);
        ...
        $DB->update_record('datac', array('id' => $data3->id, 'approval' => 1));
        ...
        $DB->update_record('datac_records', array('id' => $data3record1id, 'approved' => 1));
        ...
        $data4record2id = $this->create_default_datac_record($data4);
    ...
    public function test_attach_files() {
        ...
        $data1filefieldid = $DB->insert_record('datac_fields', $record);
        ...
        $data1textfieldid = $DB->insert_record('datac_fields', $record);
        ...
        $data1textareafieldid = $DB->insert_record('datac_fields', $record);
        ...
        $data1record1id = $DB->insert_record('datac_records', $record);
        ...
        $DB->insert_record('datac_content', $record);
        ...
        $record->dataid = $data1->id;
        $record->groupid = 0;
        $data1record2id = $DB->insert_record('datac_records', $record);
     ...
     * @param mod_datac $data
     * @return void
     */
    protected function create_default_datac_fields($fieldtypes = array(), $data) {
        $count = 1;
        ...
        foreach ($fieldtypes as $fieldtype) {
            ...
            ${$fieldname} = $this->getDataGenerator()->get_plugin_generator('mod_datac')->create_field($record, $data);
            $count++;
        }
    }
     ...
     * @return int
     */
    protected function create_default_datac_record($data, $groupid = 0) {
        global $DB;
        ...
        return $this->getDataGenerator()->get_plugin_generator('mod_datac')->create_entry($data, $fieldcontents, $groupid);
    }
    ...
    protected function get_entry_for_id($recordid ) {
        ...
        $sql = "SELECT dr.*, d.course
                  ...
                  JOIN {datac} d ON d.id = dr.dataid
                 WHERE dr.id = :drid";
