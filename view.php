...
/// These can be added to perform an action on a record
    ...
    if ($id) {
        if (! $cm = get_coursemodule_from_id('datac', $id)) {
            print_error('invalidcoursemodule');
        }
        if (! $course = $DB->get_record('course', array('id'=>$cm->course))) {
            print_error('coursemisconf');
        }
        if (! $data = $DB->get_record('datac', array('id'=>$cm->instance))) {
            print_error('invalidcoursemodule');
        }
        $record = NULL;

    } else if ($rid) {
        if (! $record = $DB->get_record('datac_records', array('id'=>$rid))) {
            print_error('invalidrecord', 'datac');
        }
        ...
        if (! $data = $DB->get_record('datac', array('id'=>$d))) {
            print_error('invalidid', 'datac');
        }
        if (! $course = $DB->get_record('course', array('id'=>$data->course))) {
            print_error('coursemisconf');
    ...
    require_capability('mod/datac:viewentry', $context);

/// If we have an empty Database then redirect because this page is useless without data
    if (has_capability('mod/datac:managetemplates', $context)) {
        if (!$DB->record_exists('datac_fields', array('dataid'=>$data->id))) {      // Brand new database!
            redirect($CFG->wwwroot.'/mod/datac/field.php?d='.$data->id);  // Redirect to field entry
        }
    }
...
/// Check further parameters that set browsing preferences
    ...
    if (!empty($advanced)) {
        ...
        list($search_array, $search) = datac_build_search_array($data, $paging, $search_array);
...
// Initialize $PAGE, compute blocks
    $PAGE->set_url('/mod/datac/view.php', $urlparams);
...
/// RSS and CSS and JS meta
    ...
    if (!empty($CFG->enablerssfeeds) && !empty($CFG->data_enablerssfeeds) && $data->rssarticles > 0) {
        ...
        rss_add_http_header($context, 'mod_datac', $data, $rsstitle);
    }
    if ($data->csstemplate) {
        $PAGE->requires->css('/mod/datac/css.php?d='.$data->id);
    }
    if ($data->jstemplate) {
        $PAGE->requires->js('/mod/datac/js.php?d='.$data->id, true);
    }
...
/// Print the page header
    ...
    if ($PAGE->user_allowed_editing()) {
        ...
        $url = new moodle_url($CFG->wwwroot.'/mod/datac/view.php', array('id' => $cm->id, 'edit' => $urlediting));
    ...
    if ($record and !datac_can_view_record($data, $record, $currentgroup, $canmanageentries)) {
        print_error('notapproved', 'datac');
    }
    ...
    /*if (!empty($CFG->enablerssfeeds) && !empty($CFG->data_enablerssfeeds) && $data->rssarticles > 0) {
        ...
        rss_print_link($context->id, $USER->id, 'mod_datac', $data->id, get_string('rsstype'));
        echo '</div>';
    ...
    $returnurl = $CFG->wwwroot . '/mod/datac/view.php?d='.$data->id.'&amp;search='.s($search).'&amp;sort='.s($sort).'&amp;order='.s($order).'&amp;';
    groups_print_activity_menu($cm, $returnurl);

/// Delete any requested records

    if ($delete && confirm_sesskey() && (datac_user_can_manage_entry($delete, $data, $context))) {
        if ($confirm = optional_param('confirm',0,PARAM_INT)) {
            if (datac_delete_record($delete, $data, $course->id, $cm->id)) {
                echo $OUTPUT->notification(get_string('recorddeleted','datac'), 'notifysuccess');
            }
            ...
            if ($deleterecord = $DB->get_record_sql("SELECT dr.*, $allnamefields
                ...
                if ($deleterecord->dataid == $data->id) {                       // Must be from this database
                    $deletebutton = new single_button(new moodle_url('/mod/datac/view.php?d='.$data->id.'&delete='.$delete.'&confirm=1'), get_string('delete'), 'post');
                    echo $OUTPUT->confirm(get_string('confirmdeleterecord','datac'),
                            $deletebutton, 'view.php?d='.$data->id);

                    $records[] = $deleterecord;
                    echo datac_print_template('singletemplate', $records, $data, '', 0, true);
    ...
    if ($multidelete && confirm_sesskey() && $canmanageentries) {
        if ($confirm = optional_param('confirm', 0, PARAM_INT)) {
            foreach ($multidelete as $value) {
                datac_delete_record($value, $data, $course->id, $cm->id);
            }
        } else {
            $validrecords = array();
            $recordids = array();
            ...
            $action = new moodle_url('/mod/datac/view.php', $submitactions);
            $cancelurl = new moodle_url('/mod/datac/view.php', array('d' => $data->id));
            $deletebutton = new single_button($action, get_string('delete'));
            echo $OUTPUT->confirm(get_string('confirmdeleterecords', 'datac'), $deletebutton, $cancelurl);
            echo datac_print_template('listtemplate', $validrecords, $data, '', 0, false);
            echo $OUTPUT->footer();
            exit;
        }
    }
    ...
    list($showactivity, $warnings) = datac_get_time_availability_status($data, $canmanageentries);

    if (!$showactivity) {
        $reason = current(array_keys($warnings));
        echo $OUTPUT->notification(get_string($reason, 'datac', $warnings[$reason]));
    }
...
if ($showactivity) {
    ...
    if ($mode == 'asearch') {
        $maxcount = 0;
        datac_print_preference_form($data, $perpage, $search, $sort, $order, $search_array, $advanced, $mode);
        ...
        if (($approve || $disapprove) && confirm_sesskey() && $approvecap) {
            ...
            if ($approverecord = $DB->get_record('datac_records', array('id' => $recordid))) {   // Need to check this is valid
                if ($approverecord->dataid == $data->id) {                       // Must be from this database
                    datac_approve_entry($approverecord->id, $newapproved);
                    $msgkey = $newapproved ? 'recordapproved' : 'recorddisapproved';
                    echo $OUTPUT->notification(get_string($msgkey, 'datac'), 'notifysuccess');
                }
            }
        }
    ...
    /// Check the number of entries required against the number of entries already made (doesn't apply to teachers)
        if ($data->entriesleft = datac_get_entries_left_to_add($data, $numentries, $canmanageentries)) {
            $strentrieslefttoadd = get_string('entrieslefttoadd', 'datac', $data);
    ...
    /// Check the number of entries required before to view other participant's entries against the number of entries already made (doesn't apply to teachers)
        ...
        if ($data->entrieslefttoview = datac_get_entries_left_to_view($data, $numentries, $canmanageentries)) {
            $strentrieslefttoaddtoview = get_string('entrieslefttoaddtoview', 'datac', $data);
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            datac_search_entries($data, $cm, $context, $mode, $currentgroup, $search, $sort, $order, $page, $perpage, $advanced, $search_array, $record);
        ...
        if (empty($records)) {
            if ($maxcount){
                ...
                echo $OUTPUT->notification(get_string('foundnorecords','datac', $a));
            } else {
                echo $OUTPUT->notification(get_string('norecords','datac'));
            }

        } else {
            //  We have some records to print.
            $url = new moodle_url('/mod/datac/view.php', array('d' => $data->id, 'sesskey' => sesskey()));
            ...
            if ($maxcount != $totalcount) {
                ...
                $a->max = $maxcount;
                $a->reseturl = "view.php?id=$cm->id&amp;mode=$mode&amp;search=&amp;advanced=0";
                echo $OUTPUT->notification(get_string('foundrecords', 'datac', $a), 'notifysuccess');
            }
            ...
            if ($mode == 'single') { // Single template
                ...
                if (empty($data->singletemplate)){
                    echo $OUTPUT->notification(get_string('nosingletemplate','datac'));
                    datac_generate_default_template($data, 'singletemplate', 0, false, false);
                }

                //datac_print_template() only adds ratings for singletemplate which is why we're attaching them here
                ...
                if ($data->assessed != RATING_AGGREGATE_NONE) {
                    ...
                    $ratingoptions->returnurl = $CFG->wwwroot.'/mod/datac/'.$baseurl;
                ...
                datac_print_template('singletemplate', $records, $data, $search, $page, false, new moodle_url($baseurl));
                ...
                if (empty($data->listtemplate)){
                    echo $OUTPUT->notification(get_string('nolisttemplate','datac'));
                    datac_generate_default_template($data, 'listtemplate', 0, false, false);
                }
                echo $data->listtemplateheader;
                datac_print_template('listtemplate', $records, $data, $search, $page, false, new moodle_url($baseurl));
            ...
            if ($mode != 'single' && $canmanageentries) {
                ...
                $module = array('name' => 'mod_datac', 'fullpath' => '/mod/datac/module.js');
                $PAGE->requires->js_init_call('M.mod_datac.init_view', null, false, $module);
            }
    ...
    if ($mode == '' && !empty($CFG->enableportfolios) && !empty($records)) {
        ...
        if (has_capability('mod/datac:exportallentries', $context) || has_capability('mod/datac:exportentry', $context)) {
            $canexport = true;
        } else if (has_capability('mod/datac:exportownentry', $context) &&
                $DB->record_exists('datac_records', array('userid' => $USER->id))) {
            $canexport = true;
        }
        if ($canexport) {
            require_once($CFG->libdir . '/portfoliolib.php');
            $button = new portfolio_add_button();
            $button->set_callback_options('datac_portfolio_caller', array('id' => $cm->id), 'mod_datac');
            if (datac_portfolio_caller::has_files($data)) {
